                    ...
                    for (int j = 0; j < dt.Columns.Count; j++)
                    {
                        ...
                        if (dt.Rows[i][j] != null)
                        {
                            ...
                            else
                            {
                                ...
                                var value = 1234456676666565;// 1234567890;
                                                             //if (j == 0)
                                                             //wr.Write(value.ToString("#") + "\t");
                                                             //else if (j == 2)
                                                             //    wr.Write(String.Format("{0:#}", value) + "\t");
                                                             //else
                                                             //    wr.Write(" " + 1234456676666565 + " " + "\t");

                                if (j == 0)
                                    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", " " + (i + 1)) + "\t");
                                  else
                                    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", " " + dt.Rows[i][j].ToString().Replace("\"", "").Replace("\r\n", "").Replace("\n", "").Replace("\n", "").Replace("\t", "")) + "\t");
