...
namespace SimplrExportBLL
{
    public class Export
    {
        ...
        //public string GetStaffExcelReport(DataTable dt, string fileLocation, string fileName, string TblName)
        //{
        //    try
        //    {
        ...
        //            var endDate = Convert.ToDateTime(toDate).AddHours(23).AddMinutes(59).AddSeconds(59);

        //            var result = (from m in _context.Tb_Staff
        //                          join dc in _context.Tb_DepartmentConfigure on m.StaffId equals dc.StaffId
        //                          join se in _context.Tb_StaffEducation on m.StaffId equals se.StaffId
        //                          join sw in _context.Tb_StaffWorkExperience on m.StaffId equals sw.StaffId
        //                          where
        //                          (m.DateOfJoining >= startDate && m.DateOfJoining <= endDate)
        //                          || dc.Designation == designation
        //                          || dc.Department == department
        //                          || se.Qualification == (qualification != null ? qualification : string.Empty)
        //                          select new
        //                          {
        //                              StaffId = m.StaffId,
        //                              UniqueId = dc.UniqId,
        ...
        //                              Address = m.Address,
        //                              Gender = m.Gender,
        //                              DateOfBirth = m.DateOfBirth.ToString("dd-MM-yyy"),
        //                              DateOfJoining = m.DateOfJoining.ToString("dd-MM-yyyy"),
        //                              EMail = m.EmailId,
        //                              AadharNumber = m.AdarCardNumber,
        //                              Department = dc.Department,
        //                              Designation = dc.Designation,
        //                              Qualification = se.Qualification,
        //                              PreviousCompany = sw.OrganizationName,
        //                              PlaceWorked = sw.Place,
        //                              isActive = m.isActive
        //                          }).ToList();

        //            var worksheet = workbook.Worksheets.Add("StaffReport");

        //            var columnNames = GetFieldNames(result.FirstOrDefault());
        ...
        //                fileDownloadName: "StaffReport_" + fromDate.Replace("\"", "") + "_" + toDate.Replace("\"", "") + ".xlsx");
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        private string print2Dirs(DataTable dt, string fileLocation, string fileName, string TblName)
        {
            var drr = "";
            var strSteps = "AutoExport 1";
            string tempdir = FolderPath + fileName;
            ...
            ////{
            //    ExcelWorksheet worksheet = workbook.Worksheets.Add(dt.TableName);
            ...
            //workbook.Save(FolderPath + fileName + ".xlsx");
            //}
            //catch (Exception ex)
            //{
            ...
            StreamWriter wr = new StreamWriter(FolderPath + fileNamePath + "\\" + tempfileName + ".xls",
                true, System.Text.Encoding.Unicode);// Encoding.Unicode);// System.Text.Encoding.UTF32);
            ...
            for (int i = 0; i < (dt.Rows.Count); i++)
            {
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    ...
                    if (dt.Rows[i][j] != null)
                    {
                        if (dt.Rows[i][j].ToString().Split('/').Length == 3)//3)// && dt.Rows[i][j].ToString().Split('/')[2].Length == 4)
                        {
                            //wr.Write(" " + dt.Rows[i][j].ToString().Replace("\"", "").Replace("\r\n", "").Replace("\n", "").Replace("\n", "").Replace("\t", "") + "\t");
                        ...
                        {
                            Double valu = 123456789865;
                            Double valu1 = 333456789865;
                            fieldValue = dt.Rows[i][j].ToString().Replace("\"", "").Replace("\r\n", "").Replace("\n", "").Replace("\n", "").Replace("\t", "");
                            //if (j == 0)
                            //    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", (i + 1).ToString()) + "\t");
                            //if (j == 2)
                            //    wr.Write(String.Format(" {0}", valu) + "\t");
                            //if (j == 3)
                            //    wr.Write(String.Format(" {0}", "rfrefefre ,frrf") + "\t");
                            //else
                            //    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", fieldValue) + "\t");
                            //if (j == 1)
                            //    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", valu) + "\t");
                            //else if (j == 2)
                            //    //wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", 123456789865).Replace(',',' ') + "\t");
                            //    wr.Write(" " + valu1.ToString(System.Globalization.CultureInfo.CurrentCulture) + " " + "\t");
                            //else
                            //    wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:#},", valu) + "\t");



                            //wr.Write(String.Format(System.Globalization.CultureInfo.InvariantCulture, "{0:N}", valu1) + "\t");
                            //wr.Write(" " + valu1.ToString(" ", System.Globalization.CultureInfo.CurrentCulture) + " " + "\t");

                            //wr.Write(String.Format("{0:###0.##}", 123145678905465));
                            var ddd = string.Format(new System.Globalization.CultureInfo("en-GB"), "{0:C}", 12345786565);
                            if (j == 0)
                                wr.Write((i + 1) + "\t");
                            //if (j == 3)
                            //    wr.Write(123456789012 + "\t");
                              if (j == 1)
                            {
                                var fff = string.Format(new System.Globalization.CultureInfo("en-GB"), "{0}", 12345786565);
                                wr.Write(fff + "\t");
                            }
                            //else if (j == 1)
                            //{
                            //    var fff = string.Format(new System.Globalization.CultureInfo("en-GB"), "{0}", fieldValue);
                            //    wr.Write(fff + "\t");
                            //}
                            else
                            {
        ...
        private string dttoexcel(DataTable dt, string fileLocation, string fileName, string TblName)
        {
            ...
            wb.Style.Fill.PatternType = XLFillPatternValues.None;
        ...
        {
            var dd = print2Dirs(dt, fileLocation, fileName, TblName);
            return dd;
            var drr = "";
            var strSteps = "AutoExport 1";
            string tempdir = FolderPath + fileName;
            ...
            try
            {
                ...
                StreamWriter wr = new StreamWriter(FolderPath + fileNamePath + "\\" + tempfileName + ".xls", true, System.Text.Encoding.UTF8);// Encoding.Unicode);// System.Text.Encoding.UTF32);
