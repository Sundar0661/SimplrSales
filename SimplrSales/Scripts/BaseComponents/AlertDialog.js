function show(messageCode, mButtonNames, params, len) {
...
function showAlertMessage(obj, messageHeader) {
    ...
    //changes done by vignesh on 16/09/2024 for alert size change
    //$('<div></div>').appendTo('body')
    ...
    //                });
      ...
      if (ProjectName.toLowerCase() == "fgv" || ProjectName.toLowerCase()=="ffb") {
        $('<div></div>').appendTo('body')
            .html('<div><h6>' + obj.message + '</h6></div>').dialog({
                modal: true, title: obj.title, zIndex: 10000, autoOpen: true,
                // width: 'auto', resizable: false,
                width: '25%', resizable: false, closeOnEscape: false,
                buttons: btns
            });
    }
    else {
