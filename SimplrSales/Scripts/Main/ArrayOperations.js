...
function setColorConfig(language) {
    ...
    var qry1 = '';
    dbDataRows = "";
    qry = "SELECT * FROM ColorConfig  where solutionName='" + SolutionName + "' and ScreenName like '" + _screenName + "%'  order by ScreenName, FieldName";
    qry1 = "SELECT * FROM ColorConfig  where solutionName='" + SolutionName + "' and ScreenName like '" + _screenName + "%'  order by roworder";//  order by ScreenName, FieldName";
            ...
            for (var condCtr = 0; condCtr < sCondArr.length; condCtr++) {
                ...
                if (DataMember.toUpperCase() == tmpFieldVal.toUpperCase()) {
                    ...
                    if (sCondArr[condCtr].Condition == '>') {
                        ...
                        if (ProjectName == 'FGV' && screenName == 'PONewDistributorForm' && tmpFieldVal.toUpperCase() == 'ROWCOLOR' && ConditionFieldValue == sCondArr[condCtr].ConditionValue) {
                            CForeColor = argbToRGB(sCondArr[condCtr].CForeColor);
                            if (item["Qty"] == item["SuggestedQty"]) {
                                CBackColor = 'transparent';
                            } else {
                                //CForeColor = argbToRGB(sCondArr[condCtr].CForeColor);
                                CBackColor = argbToRGB(sCondArr[condCtr].CBackColor);
