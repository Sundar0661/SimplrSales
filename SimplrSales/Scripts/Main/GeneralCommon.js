...
console.log('generalcommon.js,300,index-' +  currentRowClickCount);
                        ...
                        if (tdType == "text" || tdType == "select" || tdType == "hidden") {
                            if (dataMemberType == 'NUMBERFORMAT' || dataMemberType == 'AMOUNTNUMBERFORMAT') {
                                ...
                                tblbody.rows[currentIndex].cells.namedItem(id).childNodes['0'].value = value;
                                    ...
                                    multiSelectListLookUpChanged(id, currentIndex);
                            ...
                            if (isListLookUpClicked == true && isMultiSelect == true) {
                                ...
                                //changes done by sundar on 19/11/2024
                                //var chkColName = arrList[1].FieldName.replace("\t", "");
                                var chkColName = arrList[i].FieldName.replace("\t", "");
                                var colVal = data[i][chkColName];
                           ...
                           // var link = tmpText;
                            ...
                            if ((ProjectName == "FGV" || ProjectName == "EBFF") && scrName.toString().toLowerCase() == "newcustomerform")
                                imgPath = ServiceSaveImagePath + "/" + value;
                            ...
                            var tmpp = "../Images/noimage.png";

                            tblbody.rows[currentIndex].cells.namedItem(id).innerHTML = "<img id='" + id + '_' + currentIndex + "'  src = '" + imgPath + "' width = '" + imgheight + "' height = '" + imgheight + "' onError='doSomething(this)' > ";
                            ...
                            try {
                                ...
                                $.ajax({
                                    ...
                                    dataType: 'json',
                                    data: JSON.stringify({ 'imgName': value }), // { msg: errorStr },
                                    async: false,
                                    success: function (data) {
                                        //alert(data);

                                        if (data == false) {
                                            var im = document.getElementById(id + '_' + currentIndex); // or select based on classes
...
console.log('generalcommon.js,1408,index-' +  currentRowClickCount);
...
console.log('generalcommon.js,1434,index-' +  currentRowClickCount);
                ...
                for (var i = 0; i < formConfig.length; i++) {
                    if (formConfig[i].fieldName.toLowerCase() != 'cancelbtn' && formConfig[i].fieldName.toLowerCase() != 'savebtn') {
                        ...
                        if (formConfig[i].fieldControl.toLowerCase() == 'textbox' || formConfig[i].fieldControl.toLowerCase() == 'textarea' || formConfig[i].fieldControl.toLowerCase() == 'password') {
                            ...
                            if (data[j][formConfig[i].DataMember] == '') {
                                document.getElementById(formConfig[i].DataMember).src = "../Images/noimage.png";
                            }

                            var im = document.getElementById(formConfig[i].DataMember); // or select based on classes
                            im.onerror = function () {
                                // image not found or change src like this as default image:
...
function DynamicRowItemRemoveConfirmNew(objthis, dataMember, rowIndex, value1, fieldName, cnt, ttbody, isPerformActionCall) {
    ...
    if ((addCount > 0 && addCount > cnt && tableTotalRowCount > 0) || _isdynamic == false) {
...
function row_delete_refresh() {
    try {
        var table = document.getElementById(_ttbody);
        for (var i = 0; i < table.rows.length; i++) {
            table.rows[i].id = "Item" + i;
            // table.rows[i].className = "tablecell trRow_ListBodyDivId_InvoiceNewForm_Item_" + i;

            var tmpp = table.rows[i].attributes[0].nodeValue;
            ...
            table.rows[i].attributes[0].nodeValue = tmpp;// "rowItemClicked(this,\'' + fieldName + '\',\'' + addCount + '\' ,\'' + ttbody + '\',\'' + ttableId + '\');";
            // "keyDowned(this,'Item','" + i + "' ,'" + tbodyid + "','" + tableid + "');"
        }
    } catch (e) {
...
function formLookUpClicked(obj, dataMember, rowIndex, value, lookUpTitle, scrName) {
    ...
    document.getElementById(searchSelectedId).focus();
...
function listLookUpClicked(obj, dataMember, rowIndex, listName, lookUpTitle, ttbody, tfoot, sName) {
    ...
    currentRowClickCount = rowIndex;
console.log('generalcommon.js,2656,index-' +  currentRowClickCount);
...
function multiSelectListLookUpClicked(obj, dataMember, rowIndex, listName, lookUpTitle, ttbody, tfoot, sName) {
    ...
    currentRowClickCount = rowIndex;
console.log('generalcommon.js,2779,index-' +  currentRowClickCount);
    setListValue(obj, dataMember, rowIndex, ttbody);
    var _obj = {};
    _obj.fieldName = dataMember;
    _obj.rowIndex = rowIndex;
...
function multiSelectListLookUpChanged(dataMember, rowIndex) {
    try {
        currentRowClickCount = rowIndex;
console.log('generalcommon.js,2912,index-' +  currentRowClickCount);
        //setListValue(obj, dataMember, rowIndex, ttbody);
        var _obj = {};
        _obj.fieldName = dataMember;
        _obj.rowIndex = rowIndex;
        _obj.value = dataMember;
        _listLookUpIndex = rowIndex
        PerformAction('listLookUpChanged', _obj);
    } catch (e) {
...
function dynamicNewRowAdd() {
    ...
    if ((tmp.rows.length - 1) <= _listLookUpIndex) {
        ...
        //Commented by Vignesh on 12-09-2024 
...
function lookUpPopUpOpen() {
    ...
    try {
        _lookUpTitle = document.getElementById('lbl_Title').innerText;
    }
    catch { }
