const { active } = require("d3-transition");
...
function viewSurvey() {
    ...
    if (document.getElementById(lst_current_Theme) != null || document.getElementById(lst_current_Theme) != undefined) {
       ...
       // alert("No survey is selected or There is no survey.");
        var obj = {};
        obj.title = "Information";
        obj.message = "No survey is selected or There is no survey.";
        showAlertMessage_survey(obj);
    }
    ...
    if (selectedSurvey != '' && selectedSurvey != null) {
        ...
        try {
            $.ajax({
                ...
                dataType: 'json',
                async: false,
                data: { AnsNo: selectedAnsNo, checkListName: selectedSurvey, description: "",active: "false" },
                success: function (results) {
                    if (results != null && results != "" && results != undefined) {
            ...
            if (Answer[i].FormId.toString() == pageNumber.toString()) {
                ...
                switch (ctrltype) {
                    ...
                    case 'combobox':
                        ...
                        //Added by Vignesh on 04/11/2024
                        document.getElementById(ctrlName).innerHTML = Answer[i].Value;
                        break;
                    case 'radiobutton':
...
function PictureEvent(passedId) {
    ...
    //position:fixed
    //class="ui-state-active" 
    //Changes done by vignesh on 04/11/2024
    ...
    htm += '<img style="width:100%;height:100%;object-fit:cover;" id="img_' + id + '" />';
    ...
    //Added by vignesh on 04/11/2024
    load_uploaded_survey_Image1(id);
}

 //Added by vignesh on 04/11/2024
function load_uploaded_survey_Image1(id) {
...
function DropDownEvent(passedId) {
   ...
   // htm += '<div style="width:80%;"></div>'
    htm += '<div style="width:80%;"><select id="' + id + '" disabled style="z-index: -1;width:80%;"> </select></div>'

    //changes done by vignesh on 25/10/2024
    htm += '<div style="width:20%;float:right;"><img  style="width:25px;height:25px;" src="~/Images/Survey/dropdown_Icon.png"/></div>'
