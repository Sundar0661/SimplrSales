...
function ListConfigClearArrayList() {
    FieldNameArrayList = [];
...
function GetGridHeaderDetails(headerUrl, rowUrl, theadId, ttbody, tfoot, scrName) {
...
function GetRowData(rowUrl, ttbody, tfoot, scrName, pageNumber, SearchId) {
    // if (isListLookUpClicked != true)
...
function GetListDropDownListValue(screenName, FieldName, ttbody, index) {
    ...
    for (var i = 0; i < DropDownIdList.length; i++) {
        ...
        //$.ajax({
        //    type: 'POST',
        //    url: url_GetListConfigDropDownValue,
        //    data: { ScreenName: scrName, id: id },
        //    dataType: 'json',
        //    async: false,
        //    success: function (data) {
        //        if (data != null)
        //            populateListDropDown(data, id, cnt);
...
function GetEditForm(code, value) {
    ActionId = code;
    $.ajax({
        type: 'POST',
        url: url_GetEditFormData,
        data: { ScreenName: screenName + "_" + value, Code: code },
        dataType: 'json',
        async: false,
        success: function (data) {
            if (data != null) {
        ...
        if (listConfig[j].IsHidden != 1 && listConfig[j].ColumnWidth != "0px" && (isBtnFormPopUpTable == false || listConfig[j].FieldName.replace("\t", "").toLowerCase() != 'action')) {
            ...
            switch (fieldControl.toLowerCase()) {
                ...
                case "image":
                    ...
                    htm += '<button  id="delete" onclick="DynamicRowItemRemove(this,\'' + listConfig[j].FieldName + '\',' + addCount + ',\'' + listConfig[j].FieldName + '\',\'' + fieldName + '\',' + addCount + ');"  class="btn"><i     class="fa fa-trash"></i></button>';
...
function DynamicRowItemRemoveConfirm(objthis, dataMember, rowIndex, value1, fieldName, cnt, ttbody) {
    ...
    if (addCount > 0 && addCount > cnt && tableTotalRowCount > 1) {
        ...
        //$('.trRow_' + cnt).find('td').remove();
        //$('.trRow_' + cnt).hide();

        // addCount--;
        //objAddDynamicListCount['ListConfig_' + CurrentScreen_TabScreen_Name + '_' + fieldName] = objAddDynamicListCount['ListConfig_' + CurrentScreen_TabScreen_Name + '_' + fieldName] == undefined ? -1 :
        //    objAddDynamicListCount['ListConfig_' + CurrentScreen_TabScreen_Name + '_' + fieldName] - 1;

        ///listTextFieldLostFocus(objthis, dataMember, rowIndex, value1, fieldName);
