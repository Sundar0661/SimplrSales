...
function GetListDropDownListValue1(screenName, FieldName, ttbody, index) {
    //var cnt = 0;
    ...
    if (ProjectName == "CPF" && (currentScreenName == "CreditNoteNewForm" || currentScreenName == "GoodsReturnNewForm") && id == "ReasonCode")
        tblbody.rows[index].cells.namedItem(id).childNodes['0'].selectedIndex = "0";
}
function GetListDropDownListValue(screenName, FieldName, ttbody, index) {
    ...
    for (var i = 0; i < DropDownIdList.length; i++) {
        ...
        //if (formList == "List")
         //   mScreenName = screenName + "_LIST_COMBOBOX_" + DropDownIdList[i].DataMember;
       // else
         //   mScreenName = screenName + "_LIST_FORM_COMBOBOX_" + DropDownIdList[i].DataMember;
        ...
        if (currentScreenName == "ItemMasterLabelPrintingForm" && ProjectName == "SELASIHAMAN") {
            ...
            try {
                if (DropDownIdList[i].AutoSearch == "Yes")
                    $("#" + DropDownIdList[i].DataMember + DropDownIdList[i].RowIndex).select2();
            } catch (e) {
            ...
            if (data != null) {
                ...
                for (var n = 0; n < select.length; n++) {
                    ...
                    if (DropDownIdList[i].AutoSearch == "Yes")
                    tstCase = id + DropDownIdList[i].RowIndex;

                    if (select[n] != undefined && tstCase == select[n].id) {
                        var valueText = '';
                      ...
                      //  if (ProjectName.toLowerCase() == "frostfood")
                        ...
                        if (ProjectName == "CPF" && (currentScreenName == "CreditNoteNewForm" || currentScreenName == "CreditNoteForm" || currentScreenName == "GoodsReturnNewForm" || currentScreenName == "GoodsReturnForm" ) && id == "ReasonCode" ) {
                            ...
                            if (DropDownIdValueList.length == 0) {
                                ...
                                //if (DropDownIdValueList[i].Value != "") {
                                for (var z = 0; z < DropDownIdValueList.length; z++) {
                                    if (id == DropDownIdValueList[z].DataMember) {
                                        if (DropDownIdValueList[z].Value != '')
                                        tblbody.rows[index].cells.namedItem(id).childNodes['0'].value = DropDownIdValueList[z].Value;
                                    }
                                    }
...
function GetBase64Image(id, path) {
    if (path != "No files selected." && path != null && path.indexOf('.simg') > 0) {
        $.ajax({
            ...
            dataType: "json",
            async: false,
            data: { path1: path.replace('//', '/'), screenname: currentScreenName},
            success: function (data) {
