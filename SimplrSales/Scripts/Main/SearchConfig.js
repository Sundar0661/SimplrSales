...
function GetSearchConfig(screenName, fieldName, ttbody, tfoot) {
    ...
    //        document.getElementById('SearchDivId_' + screenName + '_' + fldName).style.width = "17%";
    //        document.getElementById('SearchOptionId_' + screenName + '_' + fldName).style.width = "40%";
    //    }
    //    else {
    //        document.getElementById('SearchDivId_' + screenName + '_' + fldName).style.width = "14%";
    //        document.getElementById('SearchOptionId_' + screenName + '_' + fldName).style.width = "43%";
    //    }
    //}
    //catch { }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null && data.toString() != "") {
                ...
                //Added by Vignesh on 10/09/2024
                if (ttbody.split('_')[ttbody.split('_').length - 1] != "GenericLookUp") {
                    fldName = ttbody.split('_')[ttbody.split('_').length - 1];
                }
    ...
    if (data != null && data.toString() != "") {
        ...
        AssignSortDropDownData(data, screenName + '_' + fldName + '', ttbody, tfoot);
        $('#SortSearchDiv_' + screenName + '_' + fldName + '').show();
...
function AssignDropDownData(data, screenName, ttbody, tfoot) {
    ...
    if (ProjectName.toLowerCase() != "sengchoon" && ProjectName.toLowerCase() != "ywf")
    ...
    if (ProjectName.toLowerCase() == "sengchoon" || ProjectName.toLowerCase() == "ywf")
        SetSearchControl(screenName, ttbody, tfoot);
    //-------------------------------------Log Start----------------------------------------//
    //try {
    //    var idd = $('#SearchFieldId_' + screenName + '').val();
    //    if (screenName == "SalesOrderList" && ProjectName == "POC")
...
function AssignSortDropDownData(data, screenName, ttbody, tfoot) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        //_searchControl1[data[i].FieldName] = searchobj;
        //_searchControl[data[i].FieldName] = data[i].SearchControl;
        _searchControl1_sort[data[i].FieldName] = searchobj;
        _searchControl_sort[data[i].FieldName] = data[i].SearchControl;
    ...
    if (event.keyCode === 13 || isDatePicker == 1 || iscombobox == 1) {//enter button click event
        ...
        setTimeout(function () {
            if ($('#SearchFieldId_' + scrName + '').val() != undefined) {
                ...
                //Added by Vignesh on 10/09/2024
                if (ttbody.split('_')[ttbody.split('_').length - 1] != 'GenericLookUp') {
                    if (ttbody.split('_')[ttbody.split('_').length - 1] != FieldName) {
                        searchOptionArray = [];
                        FieldName = ttbody.split('_')[ttbody.split('_').length - 1];
                    }
                }
...
function SetSearchControl(screenName, ttbody, tfoot) {
    ...
    if (selectedvalue == "TEXTBOX") {
        ...
        //Changes done by vignesh on 12/09/2024
        html = '<input  autocomplete="off" class="searchdatepicker" type="text" id="Search_' + screenName + '_' + selectedId + '"  onchange="EnterSearch(\'' + screenName + '\',\'' + selectedvalue1.FieldControl + '\',\'' + ttbody + '\',\'' + tfoot + '\',1);"  onkeyup="EnterSearch(\'' + screenName + '\',\'' + selectedvalue1.FieldControl + '\',\'' + ttbody + '\',\'' + tfoot + '\',2);" style="font-size: 16px;" />';
        $(html).appendTo('#SearchOptionId_' + screenName + '');
        SearchDatePicker("No");
    }
...
function ListSearch(screenName, FieldControl, selectedId, ttbody, tfoot, onblur) {
    ...
    if ($('#SearchFieldId_' + screenName + '').val() != undefined) {
        if (isListLookUpClicked == true) {
            ...
            FormListConfigRow("ListBodyDivId_" + CurrentScreen_TabScreen_Name + "_" + FieldName, "ListfootDivId_" + CurrentScreen_TabScreen_Name + "_" + FieldName, CurrentScreen_TabScreen_Name, 1, $('#SearchFieldId_' + screenName + '').val(), FieldName);
        }
    }
    else
