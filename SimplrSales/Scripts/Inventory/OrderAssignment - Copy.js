...
function MoveOrder(vehicleId, ordNo, isAction) {
    ...
    if (executeQry[0].DeliQty == 0 && executeQry[0].OrdLevelstatus != "PICK") {
        if (vehicleId.toLowerCase() == "buffer" || vehicleId.toLowerCase() == "bufferma" || vehicleId.toLowerCase() == "spc")
            qry = "  Update Orderhdr set AgentID='',VehicleID='',OrdLevelStatus='PLG' where OrdNo='" + ordNo + "' ";
        else
            qry = "  update Orderhdr set AgentId='" + vehicleId + "',  VehicleId='" + vehicleId + "' ,  DTG='" + dtg + "',OrdLevelStatus=Case when OrdLevelStatus='PLG' then 'PLA' else OrdLevelStatus end where OrdNo='" + ordNo + "' ";
...
function setIntervalFunction() {
    var qry = "SELECT CAST((CONVERT(bigint, MAX(ts))) as decimal)  as ts FROM salesagent union SELECT CAST((CONVERT(bigint, MAX(ts))) as decimal)  as ts FROM OrderHdr";
    execute(qry);
    if (executeQry != null) {
...
function DDAssignVehicle(data) {
    ...
    $.each(data, function (j, data) {
        ...
        if (data.Code.toLowerCase() != "buffer" && data.Code.toLowerCase() != "bufferma" && data.Code.toLowerCase() != "spc")
            valueText1 += "<option value=" + data.Code + " id=" + data.Code + ">" + data.UserID + "</option>";
    });
    $("#VehicleId").append(valueText);
    $("#VehicleId1").append(valueText1);
}
...
function LoadVehicle(data) {
    ...
    if (data != null) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            htm += '<div  id="divbackRoundColor' + i + '"  style="background-color: ' + BackRoundcolor + '; height: 40px;margin-top:-1px">';
            ...
            if (data[i].Code.toLowerCase() != "buffer" && data[i].Code.toLowerCase() != "bufferma" && data[i].Code.toLowerCase() != "spc") {
            ...
            htm += '<th  style="width: 18%;">ToteBox</th>';
    ...
    $.ajax({
        ...
        success: function (results) {
            ...
            for (var i = 0; i < vehicleData.length; i++) {
                ...
                htm += '<th  style="width: 16%;;">Order No</th>';
                htm += '<th  style="width: 12%;">ToteBox</th>';
                htm += '<th  style="width: 12%;;">Total Lines</th>';
                //htm += '<th  style="width: 35%;;">Customer No</th>';
                htm += '<th  style="width: 19%;;">Customer Name</th>';

                htm += '<th  style="width: 12%;;">Plan DeliveryDate</th>';
                htm += '<th  style="width: 12%;;">Ship to Country</th>';
                htm += '<th  style="width: 15%;;">Remarks</th>';
                ...
                for (var j = 0; j < selectData.length; j++) {
                    ...
                    if (vehId == selectData[j].VehicleID) {
                        ...
                        htm += '<td style="display: none">' + selectData[j].OrdNo + '</td>';
                        htm += '<td  onclick="Row_Click_Redirect(\'' + selectData[j].OrdNo + '\',\'' + selectData[j].DCOrdNo + '\')"  style="width: 16%;">' + selectData[j].DCOrdNo + '</td>';
                        //htm += '<td   style="width: 35%;">' + selectData[j].CustNo + '</td>';
                        htm += '<td   style="width: 12%;">' + selectData[j].ToteBoxno + '</td>';
                        htm += '<td   style="width: 12%;">' + selectData[j].TotalLines + '</td>';
                        htm += '<td   style="width: 19%;">' + selectData[j].CustomerName + '</td>';
                        htm += '<td   style="width: 12%;">' + selectData[j].DeliveryDate + '</td>';
                        htm += '<td   style="width: 12%;">' + selectData[j].ShipCountry + '</td>';
                        htm += '<td   style="width: 15%;">' + selectData[j].Remarks + '</td>';
                        htm += '<td   style="width: 12%;">' + selectData[j].OrdLevelstatus + '</td>';
...
function updateCancelOrder(ordNo) {
    ...
    var qry = "";
    if (executeQry[0].DeliQty == 0 && executeQry[0].OrdLevelstatus != "PICK") {
        qry = "update Orderhdr set OrdLevelstatus=case when OrdLevelstatus='PLG' then 'CAN' else 'WCN' end where OrdNo='" + ordNo + "' ;";
