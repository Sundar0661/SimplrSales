...
function SaveImageUpload(id, imgname) {
    if (window.FormData !== undefined) {
        ...
        try {
            if (ProjectName == "PVMB" && currentScreenName == "MessageConfigNewForm") {

                var numberOfMillisecondsSinceEpoch = Date.now();
                const previousFile = fileUpload.files[0];
                var tmpp = fileUpload.files[0].name.split('.');
                var newName = tmpp[0] + '_' + _UserID + '_' + numberOfMillisecondsSinceEpoch + '.' + tmpp[1];
       ...
       // oldImageName = files[0].name;
        ...
        $.ajax({
            ...
            url: url_UploadFiles + "?imgname=" + imgname + "&oldImageName=" + oldImageName + "&imgPath=" + $('#ImagePath').val() + "&screenName=" + currentScreenName,
