...
function getWaypointsDirectionsLatLonValue(action) {
    ...
    for (var i = 0; i < executeQry1.length; i++) {
        var latLon = { lat: parseFloat(executeQry1[0].Latitude), lng: parseFloat(executeQry1[0].Longitude) };
        // arrLoc.push(latLon);
        arrLoc.push(executeQry1[i].Latitude + ',' + executeQry1[i].Longitude);
    }
    ...
    directionsService.route(request, function (response, status) {
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            for (var i = 0; i < arrLoc.length; i++) {
                ...
                createMarkerNew(myLatLng, arrData[i].MarkerNo, arrData[i].Remarks + arrData[i].Distance, arrData[i].Color);
            }
...
function getMarkerImage(iconColor) {
    ...
    if (iconColor == "Start") {
        icons[iconColor] = new google.maps.MarkerImage(
            "../Images/" + iconColor + "m.png",//"http://maps.google.com/mapfiles/ms/micons/" + iconColor + ".png",
            //"mapIcons/marker_" + iconColor + ".png",
            // This marker is 20 pixels wide by 34 pixels tall.
            new google.maps.Size(80, 80),
            // The origin for this image is 0,0.
            new google.maps.Point(0, 0),
            // The anchor for this image is at 6,20.
            new google.maps.Point(3, 50));
    }
    else {
        icons[iconColor] = new google.maps.MarkerImage(
            "../Images/" + iconColor + "m.png",//"http://maps.google.com/mapfiles/ms/micons/" + iconColor + ".png",
