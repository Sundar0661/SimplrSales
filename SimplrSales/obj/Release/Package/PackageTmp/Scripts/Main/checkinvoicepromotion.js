...
function CheckInvoicePromotion(sitemid, dAmt, orderdate) {
    dAmt = dAmt.replace("'", "").replace("'", "")

    try {
        var query = "DELETE FROM temppromooffer";
    ...
    for (var t = 0; t < qry.length; t++) {
        ...
        ParamJS.GroupPromo = qry[t]['EntitleType'].replace("Per ", "");
    ...
    for (var iIndex = 0; iIndex < Arr.length; iIndex++) {
        ...
        Qry1 = GetQueryString("GET_PROMOLINETYPE");

        if (Qry1 != '')// != undefined && Qry1 != null && Qry1 !='')
        {
            var Qrye = Qry1;

            ParamJS.CLineType = Qrye[0]['CLineType'];
            ParamJS.OLineType = Qrye[0]['OLineType'];

        }

        Qry1 = GetQueryString("GET_PROMOITEMCHECK");
        ...
        if (Qry1 != '')// != undefined && Qry1 != null && Qry1 !='')
        {
            ...
            if (Qrye.length > 0)
                ...
                continue NextRecord;
            //ParamJS.LineType = Qrye[0]['LineType'];
        ...
        for (var z = 0; z < qry.length; z++) {
            ...
            try {
                ...
                if (Number(qry[z]['FOcQty']) > 0 && qry.length > 1) {
                    // bFocPromo = true;
                    var query = "insert into TempPromoOffer (PromoID, ItemID, FOCQty,GroupId,UserID) VALUES ('" + ipdDet.PromoId + "','" + qry[z]['ItemNo'] + "','" + qry[z]['FOcQty'] + "','" + qry[z]['PromoGroup'] + "','" + agentId + "')";
            ...
            if (tblOffer == 'yes') {
                ipdDet.DisAmt = qry[z]['DisPrice'];
                ipdDet.DisPer = qry[z]['Discount'];
            }
            ...
            if (bEnableFOCPrice == true) {
                ...
                var price = this.getPrice(qry[z]['ItemId'], qry[z]['Uom'], Math.floor(iMultiCnt * parseFloat(qry[z]['FOcQty'])), 1, qry[z]['Uom'], priceGroup, false);// custDet[0]["PriceGroup"], false);
    ...
    try {
        ...
        _varObj = {};
        bPromoArr = [];
        if ((resObj.DisPer != null && resObj.DisPer != undefined && resObj.DisPer != '' && resObj.DisPer != 0) || (resObj.DisAmt != null && resObj.DisAmt != undefined && resObj.DisAmt != '' && resObj.DisAmt != 0)) {
        ...
        if (resObj.DisAmt != null && resObj.DisAmt != undefined && resObj.DisAmt != '' && resObj.DisAmt != 0) {
            ...
            if (resObj.promoObj.length > 0) {
                for (var j = 0; j < resObj.promoObj.length; j++) {
                    ...
                    var query_promotemp = "insert into PromoEntitlementTemp (CustNo, OrdNo, OrderDate, PromoId, PromoCount, PromoDiscount, Userid, ItemNo) VALUES ('" + custNo + "', '" + orderno + "', '" + currentDateTime + "', '" + promoObj.Promotion + "', " + promoObj.PromoCount + ", " + resObj.DisAmt + ", '" + agentid + "','" + sitemid + "')";
...
function getOrderNo() {
    var tmp = document.getElementById("OrdNo").value;
    return tmp
}
...
function getItems() {
    var items = "";
    var ttbody = document.getElementById("ListBodyDivId_" + currentScreenName + "_Item");

    for (var i = 0; i < ttbody.rows.length - 1; i++) {
        if (items == '')
            items = ttbody.rows[i].cells.namedItem('ItemNo').childNodes['0'].value;
        else
            items = items + "," + ttbody.rows[i].cells.namedItem('ItemNo').childNodes['0'].value;
    }
    return items;
