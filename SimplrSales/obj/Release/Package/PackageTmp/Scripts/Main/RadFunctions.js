...
function CheckItemPromotion(sitemid, suom, sysCustId, scat, qty, dAmt) { //function CheckInvoicePromotion(sitemid, suom, dqty, scat, dtorderdate) {//ItemPromotion --function CheckItemPromotion(sitemid, suom, dqty, scat,qty,dAmt)
    ...
    try {
        ...
        if (Arr.length == 0) {
            //return promotion;
            promotion = CheckInvoicePromotion(sitemid, dAmt, dtorderdate);
            return promotion;
        } NextRecord:
        for (var iCnt1 = 0; iCnt1 < Arr.length; iCnt1++) {
            ...
            for (var i = 0; i < dbDataRows.length; i++) {
                ...
                for (var j = 0; j < tempOrdParse.length; j++) {
                    if ((dbDataRows[i]["MinQty"] > tempOrdParse[j]["Qty"] || dbDataRows[i]["MaxQty"] < tempOrdParse[j]["Qty"]) && (dbDataRows[i]["MinAmt"] > tempOrdParse[j]["Amount"] || qryParse[i]["MaxAmt"] < tempOrdParse[j]["Amount"])) {
                        bCondition = false;
                        _obj = {};
...
function CheckData(qry) {
    bFlag = false;
    execute(qry); //app.ExecuteSQL(qry)
...
function GetQueryString(sScreenName) {
    ...
    if (sScreenName == "GET_PROMOSELECT")
        qry = qry.replace("'',''", "','");
...
function ApplyPromotion(sPromoId, Multiplier, sItemNo, custNo) {
    ...
    for (var i = 0; i < getPriceParse.length; i++) {
        ...
        if (Number(getPriceParse[i]['DisPrice']) > 0) {
        ...
        else if (getPriceParse[i]['DisCalc'] != '') { //todo1 -- && false
            var sDisCalc = 'PRICE';
            ...
            if (getPriceParse[i]['LineType'] == 'Item') {
                ...
                for (var w = 0; w < getItemParse.length; w++) {
                    var query = "insert into TempPromoOffer (PromoID, ItemID, FOCQty,GroupId) VALUES ('" + getPriceParse[i]['PromoID'] + "','" + getItemParse[w]['itemid'] + "','" + getPriceParse[i]['FocQty'] + "','" + getPriceParse[i]['ItemID'] + "')";
                    var obj = execute(query);
                ...
                if (sTmpPromoOffer.length != 0) {
                    ...
                    qry = "insert into TempOrderdet (OrderNo, itemid, itemname, uom, qty, price, amount, PromoID, Priority, PromoOffer, DisPer, DisPr, ActPrice, ReasonCode, Remarks, LineNum, SalesType, SugQty, AutoNum, Unloaded) "
                        + "Select 'temp', P.ItemNo, P.itemname,  '" + getPriceParse[i]['UOM'] + "' as UOM, '" + getPriceParse[i]['FocQty'] + "'*" + Multiplier + " as Qty, 0 as Price, 0 as Amount, '" + getPriceParse[i]['PromoId'] + "' as PromoID, 0 as Priority,  '" + getPriceParse[i]['PromoId'] + "' as PromoOffer, 0 as DisPer, 0 as DisPr, 0 as ActPrice, isnull(Reason.Code,'FOC') as ReasonCode, '' as Remarks, 99 as LineNum, 'F' as SalesType, 0 as SugQty, 99999 as AutoNum, 0 as Unloaded from Item P Inner Join UOM ON P.ItemNo = UOM.ItemNo and UOM.UOM = '" + getPriceParse[i]['UOM'] + "' LEFT JOIN Reason ON Reason.code = '" + getPriceParse[i]['Reason'] + "' where P.ItemNo = '" + getPriceParse[i]['Itemid'] + "'";
                    //qry = "insert into TempOrderdet (OrderNo, itemid, itemname, uom, qty, price, amount, PromoID, Priority, PromoOffer, DisPer, DisPr, ActPrice, ReasonCode, Remarks, LineNum, SalesType, SugQty, AutoNum, Unloaded) "
                ...
                for (var t = 0; t < getModqry.length; t++) {
                    var query = "insert into TempPromoOffer (PromoID, ItemID, FOCQty,GroupId) VALUES ('" + getPriceParse[i]['PromoID'] + "','" + getModqry[t]['itemid'] + "','" + getPriceParse[i]['FocQty'] + "','" + getPriceParse[i]['ItemID'] + "')";
                    var obj = execute(query);
