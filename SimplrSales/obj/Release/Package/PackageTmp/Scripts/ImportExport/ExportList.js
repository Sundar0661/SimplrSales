...
function executeReader1() {
    ...
    //listQry = " Select 'C.CompanyCode' as 'Company Code','C.CompanyDesc' as 'Company Desc','C.CompanyClassification' as 'Company Classification','C.SalesOrg' as 'Sales Org','CS.SiteCode' as 'Company Site Code','CS.SiteName' as 'Company Site Name','CS.SiteType' as 'Company Site Type','CS.Region' as 'Region','CS.Province' as 'Province/City','CS.Address' as 'Address','CS.Longitude' as 'Longitude','CS.Latitude' as 'Latitude' "
...
function exportFileToSQLFromToDateJSU() {
    ...
    var fileType = $('#FileType').val() == undefined ? "XLSX" : $('#FileType').val();
...
function exportFileToSQLFromToDate() {
    ...
    if (ProjectName.toLowerCase() == "uic" || ProjectName.toLowerCase() == 'bangala'
        ...
        var branch = $('#Branch').val() == undefined ? "" : $('#Branch').val();
        var Location = $('#Location').val() == undefined ? "" : $('#Location').val();
        var agntID = $('#AgentId').val() == undefined ? "" : $('#AgentId').val();
        ...
        if (dteformat == "dd/mm/yyyy") {
            ...
            if (ProjectName.toLowerCase() == "pvmigt")
                link1 = url_ExportSQLFromToDate + "?tableName=" + postData + "&AgentID=" + $('#AgentId').val() + "&FromDt=" + fDate + "&ToDt=" + tDate + "&DistributorID=" + distributorID + "&SalesOfficeID=" + salesOfficeID + "&SalesAgent=" + salesAgent + "&FileType=" + fileType + "&Branch=" + branch;
            else if (ProjectName.toLowerCase() == "sej")
                link1 = url_ExportSQLFromToDate + "?tableName=" + postData + "&AgentID=" + $('#AgentId').val() + "&FromDt=" + fDate + "&ToDt=" + tDate + "&DistributorID=" + distributorID + "&SalesOfficeID=" + salesOfficeID + "&SalesAgent=" + salesAgent + "&FileType=" + fileType + "&Location=" + Location;
            else {
                link1 = url_ExportSQLFromToDate + "?tableName=" + postData + "&AgentID=" + agntID + "&FromDt=" + fDate + "&ToDt=" + tDate + "&DistributorID=" + distributorID + "&SalesOfficeID=" + salesOfficeID + "&SalesAgent=" + salesAgent + "&FileType=" + fileType;
                //link3 = "/Export/AExportSQLFromToDate/" + "?tableName=" + postData + "&AgentID=" + $('#AgentId').val() + "&FromDt=" + fDate + "&ToDt=" + tDate + "&DistributorID=" + distributorID + "&SalesOfficeID=" + salesOfficeID + "&SalesAgent=" + salesAgent + "&FileType=" + fileType;
                console.log(link1);
            }
        }
        else {
            ...
            if (data != "") {
                ...
                $("#table_ExportDataForm_LstExportData").find('tr').each(function (event) {
                    ...
                    if (row.find('input[type="checkbox"]').is(':checked')) {
                        ...
                        //Added By Vishnu on 20.11.2023 for Dancom
                        //if (ProjectName.toLowerCase() == "dancom" && (postData.Tolowercase() == "export invoice" || postData.Tolowercase() == "pronto accounting"))
                        if (ProjectName.toLowerCase() == "dancom")
                        {
                            hrefLable = retValue == "" ? "" : hrefUrl.split('/')[hrefUrl.split('/').length - 1];
                        }
                        else
                        {
                            hrefLable = retValue == "" ? "" : hrefUrl.split('.csv').length == 2 ? selectedIDs[j] + ".csv" : selectedIDs[j] + ".xlsx";
            ...
            if (data != "") {
                ...
                $("#table_ExportDataForm_LstExportData").find('tr').each(function (event) {
                    ...
                    if (row.find('input[type="checkbox"]').is(':checked')) {
                        ...
                        retValue = data.split(',')[j].split('$')[1];
                        hrefLable = retValue == "" ? "" : hrefUrl.split('.csv').length == 2 ? selectedIDs[j] + ".csv" : selectedIDs[j] + ".xlsx";
                        //hrefLable = retValue == "" ? "" : hrefUrl.split('.csv').length == 2 ? selectedIDs[j] + ".csv" : hrefUrl.split('.xlsx').length == 2 ? selectedIDs[j] + ".xlsx" : hrefUrl.split("//")[hrefUrl.split("//").length - 1];
                        row[0].children.Link.innerHTML = "";
                        if (retValue != "")
                            row[0].children.Link.innerHTML = "<a href='" + hrefUrl + "' style='color: blue' download>" + hrefLable + " </a>";
                        else
