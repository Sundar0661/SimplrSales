...
var comboData = [], headerTotalLength = 0, headerListLength = 0, sKeyType = 'NUMBER';
...
*/		
		...
		if (HeaderDetails != undefined && HeaderDetails != null) {
			...
			//var RowHeight = 0;
			Ti.API.info('LoadListConfig 6');
			for (var i = 0; i < HeaderDetails.length; i++) {
				Ti.API.info('LoadListConfig 7 : - ' + i + ' - ' + dLineIndex);
				...
				Ti.API.info('dLeftPos : ' + dLeftPos + ' - dTopPos : ' + dTopPos);
				...
				if(HeaderDetails[i].Header != ""){
					try{
						...
						Ti.API.info("sScreenNameTitle1 "+sScreenNameTitle);
						//db = commonObj.dbConnectionObj.createDataBaseConnection();
						dbDataRows = Ti.App.dbConn.execute('SELECT * FROM ListConfig where screenName = ' + Ti.App.SQL.safeSQL(screenName) + ' and Language = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' and FieldName = ' + Ti.App.SQL.safeSQL(HeaderDetails[i].Header) + ' order by ScreenName, DisplayNo');
						while (dbDataRows.isValidRow()) {
							sScreenNameTitle = dbDataRows.fieldByName('NewText');
							dbDataRows.next();
						}
						dbDataRows.close();
						Ti.API.info("sScreenNameTitle2 "+sScreenNameTitle);
            sScreenNameTitle = HeaderDetails[i].Header;//"";
						
						//db.close();
						for (var j = i+1; j < HeaderDetails.length; j++) {
						...
						dLeftPos += _width;
						Ti.API.info("HeaderDetails[i].HFontSize1 "+HeaderDetails[i].HFontSize);
...
//COMMON.Log('LISTCONFIG : HeaderDetails[i].headerHeight ---> ' + HeaderDetails[i].headerHeight);
						...
						for (var j = i+1; j < _LoopLen1; j++) {
							if(HeaderDetails[j].Header == _sHeader){
								Ti.API.info("HeaderDetails[j].columnText "+HeaderDetails[j].columnText);
								...
								var l2 = commonObj.BasicLabelObj.createLabel(HeaderDetails[j].columnText, ((((HeaderDetails[j].columnWidth * widthRatio * 100 / headerListLength)/_width) * 100)) + '%', HeaderDetails[j].headerHeight/2, HeaderDetails[j].HFontSize, HeaderDetails[j].HFont, HeaderDetails[j].HFontStyle, HeaderDetails[j].HForeColor, 'transparent', HeaderDetails[j].allignment, 0);
								l2.left = 0;
								l2.borderWidth = 1;
					...
					if(HeaderDetails[i].columnWidth == 0){
						...
						//label = commonObj.BasicLabelObj.createLabel(HeaderDetails[i].columnText, 0, HeaderDetails[i].headerHeight, HeaderDetails[i].HFontSize, HeaderDetails[i].HFont, HeaderDetails[i].HFontStyle, HeaderDetails[i].HForeColor, 'transparent', HeaderDetails[i].allignment, 0);
						label = commonObj.BasicLabelObj.createLabel(HeaderDetails[i].columnText, commonObj.ColumnWidth + '%', HeaderDetails[i].headerHeight, HeaderDetails[i].HFontSize, HeaderDetails[i].HFont, HeaderDetails[i].HFontStyle, HeaderDetails[i].HForeColor, 'transparent', HeaderDetails[i].allignment, 0);
...
//COMMON.Log('LISTCONFIG : HeaderDetails[i].headerHeight ---> ' + HeaderDetails[i].headerHeight);
	...
	setSearchConfig : function(language) {
		...
		//var db = commonObj.dbConnectionObj.createDataBaseConnection();
		//COMMON.Log('Search Config Start : ' + new Date());
		var SearchDataList = [], searchdata = {}, screenName = '';
		dbDataRows = Ti.App.dbConn.execute('SELECT * FROM SearchConfig where Language = ' + Ti.App.SQL.safeSQL(language) + 'order by ScreenName, DisplayNo ');
		while (dbDataRows.isValidRow()) {
		...
		dbDataRows = null;
		//COMMON.Log('Search Config End : ' + new Date());
		return true;
	},
	setColorConfig : function(language) {
		...
		var qry = "";
		dbDataRows = "";
		//COMMON.Log('Color Config Start : ' + new Date());
	...
	setListConfig : function(language) {
		...
		//COMMON.Log('List Config End : ' + new Date());
		//db = null;
		dbDataRows = null;
	...
	setMenuConfig : function(language) {
		...
		//COMMON.Log('Menu Config Start : ' + new Date());
		//qry = 'SELECT * FROM MenuConfig WHERE Visible = 1 and [Language] = ' + Ti.App.SQL.safeSQL(language) + ' order by ScreenName, DisplayNo';
		qry = 'SELECT ScreenName, MenuCode, UPPER(MenuDisplayText) as MenuDisplayText, DisplayNo, AccessLevel, Language, Visible, Popup, IsIcon, IconImage FROM MenuConfig WHERE Visible = 1 and [Language] = ' + Ti.App.SQL.safeSQL(language) + ' Group by Screenname, MenuCode order by ScreenName, DisplayNo';
		...
		menu = {}; menu = null;
		//COMMON.Log('Menu Config End : ' + new Date());
		/***** /
	...
	setMessageConfig : function(language) {
		...
		//COMMON.Log('Message Config Start : ' + new Date());
		...
		//COMMON.Log('Message Config End : ' + new Date());
	},
	...
	getMessageString : function(ScreenName, MessageCode) {
		...
		Ti.API.info('MessageObj '+ JSON.stringify(messageObj));
		return messageObj;
	},
	...
	setFormConfig : function(language) {
        ...
        try {
            ...
            //COMMON.Log('Form Config Start : ' + new Date());
            dbDataRows = Ti.App.dbConn.execute(qry);
            var iIndex = 0;
            var dFontHeightRatio = systemTableConfig['FONTRATIO'];
            dFontHeightRatio = (dFontHeightRatio == null || dFontHeightRatio == undefined || dFontHeightRatio == '') ? 1 : dFontHeightRatio;
            ...
            //COMMON.Log('Form Config End : ' + new Date());
        } catch(e) {}
    },
	...
	setQueryConfig : function() {
		...
		//COMMON.Log('Query  Config Start : ' + new Date());
		//select ScreenName, QueryText,GroupText,OrderText from QueryConfig
		dbDataRows = Ti.App.dbConn.execute("select ScreenName, QueryText, ifnull(GroupText,'') as GroupText, ifnull(OrderText, '' ) as OrderText from QueryConfig");
		while (dbDataRows.isValidRow()) {
			//screenName = dbDataRows.fieldByName('ScreenName');
        ...
        Ti.App.winfirst.fireEvent('ProgressCount');
        /*****/
		//COMMON.Log('Query  Config End : ' + new Date());
		return true;
	},
	//loadSubQuery : function(screenName) {
	//	return Titanium.App.Properties.getString('QueryConfig_' + screenName);
	//},
	setDownloadTransConfigConfig : function() {
		//var db = commonObj.dbConnectionObj.createDataBaseConnection();
		try{
			Ti.App.dbConn.execute("Update GridFunctions SET ScreenName = 'Survey List' where lower(ScreenName) = lower('Survey') and Language = 'English'");
			/*
		...
		dbDataRows = Ti.App.dbConn.execute('select * from DownloadTransConfig');
		var _obj = null;
		var arrList = [];
		//COMMON.Log('DownLoadTrans Config Start : ' + new Date());
		while (dbDataRows.isValidRow()) {
			_obj = {};
		...
		//COMMON.Log('DownLoadTrans Config End : ' + new Date());
	},
	setSystemTableConfig : function() {
		try{
			...
			//var db = commonObj.dbConnectionObj.createDataBaseConnection();
			//COMMON.Log('System Start : ' + new Date());
			...
			try{
				var sLanguage = systemTableConfig['LANGUAGE'];
				Ti.App.sLanguage = (sLanguage == null || sLanguage == undefined || sLanguage == '') ? 'English' : sLanguage;
			}catch(e){}
			
			//db.close();
			dbDataRows = null; //db = null;
			//COMMON.Log('System End : ' + new Date());
		}catch(e){}
	},
	...
	setAllConfig : function() {
       ...
       /********/ 
		...
		//COMMON.Log('setAllConfig Start : ' + new Date());
	...
	getListConfigByScreenName : function(screenName) {
        ...
        var arrTotalWidth = [], dLineIndex = 0;
        var dFontHeightRatio = systemTableConfig['FONTRATIO'];
		dbDataRows = Ti.App.dbConn.execute('select ScreenName, HeaderHeight, HFont, HFontSize, HFontStyle, DisplayNo, FieldName, DefaultText, NewText, ColumnWidth, HBackColor, HForeColor, HBackColor, RForeColor, RBackColor, RFont, RFontSize, RFontStyle, Alignment, RowHeight, FieldControl, DataMember, DataMemberType, Header, LineIndex, Showborder, BorderColor, ColumnUnit from  ListConfig  where  DataMember <> "" and Screenname = ' + Ti.App.SQL.safeSQL(screenName) + ' and [Language] = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' order by ScreenName, LineIndex, DisplayNo');
        dFontHeightRatio = (dFontHeightRatio == null || dFontHeightRatio == undefined || dFontHeightRatio == '') ? 1 : dFontHeightRatio;
        while (dbDataRows.isValidRow()) {
            screenName = dbDataRows.fieldByName('ScreenName');
            header = {};
	...
	getQueryByScreenName : function(queryName) {
		if (mView == null) {
			commonObj.query = Titanium.App.Properties.getString('QueryConfig_' + queryName);
			commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + queryName + '_GroupText');
			commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + queryName + '_OrderText');
			return commonObj.query;			
		}
		commonObj.query = Titanium.App.Properties.getString('QueryConfig_' + queryName);
		commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + queryName + '_GroupText');
		commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + queryName + '_OrderText');
		return mView.formatQueryString(commonObj.query, queryName);
	},
    ...
    //POINTOFSALES //TRAVELITE
		...
		if(screenName == 'Stock Order' || screenName == 'Loading' || screenName == 'Unloading - Items' || screenName=='Unloading - Return Items' || screenName == 'Unloading - Variance' || screenName == 'Acknowledgement' || screenName =='Partial Unloading - Items' || screenName =='Partial Unloading - Return Items' || screenName =='UnsoldDamage' || screenName == 'Van Stock Balance' || screenName == 'Partial Unloading - Variance'){
			...
			}else
			{
			 ...
			 DFChileView2Lbl3 = commonObj.BasicLabelObj.createLabel('TOTAL SKU/QTY : 0/0', '30%', '100%', parseInt(18*commonObj.dFontHeightRatio), '', 'normal', '#333', 'transparent', 2, 2);			
			}
			...
			*/			
		...
		}else if(screenName == 'Sales Items' || screenName == 'Sales' || screenName == 'ProposeOrder'){
			...
			if(commonObj.bShowSalesTotalAmount == true){
    			...
    			parent.height = 90;
    			var DFChileView = TableViewBasicUIObj.createBasicView(null, 'white', 90, '100%', null, null, null, null, 'vertical');
			}
			...
			if(commonObj.bShowSalesTotalAmount == true){
				...
				var DFChileView1Lbl1 = commonObj.BasicLabelObj.createLabel('Total SKU/Qty : 0/0', '50%', '100%', parseInt(18*commonObj.dFontHeightRatio), '', 'normal', '#333', 'transparent', 2, 2);
				var DFChileView2Lbl2 = commonObj.BasicLabelObj.createLabel('Total : 0', '50%', '100%', parseInt(18*commonObj.dFontHeightRatio), '', 'normal', '#333', 'transparent', 1, 2);

			}else{
				var DFChileView1Lbl1 = commonObj.BasicLabelObj.createLabel('Total SKU/Qty : 0/0', '100%', '100%', parseInt(18*commonObj.dFontHeightRatio), '', 'normal', '#333', 'transparent', 2, 2);
                ...
                var DFChileView2Lbl3 = commonObj.BasicLabelObj.createLabel('Total : 0', '50%', '100%', parseInt(18*commonObj.dFontHeightRatio), '', 'normal', '#333', 'transparent', 1, 2);
			}
			
			Ti.API.info('commonObj.bShowSalesTotalAmount => ' + commonObj.bShowSalesTotalAmount);
			...
			//var sFormFooterView = ArrayOperations.prototype.loadFormConfig(screenName+'_FOOTER', []);
			dbDataRows = Ti.App.dbConn.execute('SELECT * from FormConfig where Screenname = ' + Ti.App.SQL.safeSQL(screenName+'_FOOTER') + ' and Language = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' Order by DisplayNo');
			Ti.API.info('SELECT * from FormConfig where Screenname = ' + Ti.App.SQL.safeSQL(screenName+'_FOOTER') + ' and Language = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' Order by DisplayNo');
			if(dbDataRows.isValidRow()) {
				while(dbDataRows.isValidRow()) {
					//salesHeaderList.height = dbDataRows.fieldByName('HeaderHeight');
...
/********************************/		
	...
	loadFooterButtons : function(screenName) {
		...
		//setTimeout(function() {
			//Ti.API.info('2829 screenName '+screenName);
			...
			commonObj.dbDataRows = Ti.App.dbConn.execute('SELECT * FROM MenuConfig WHERE Visible = 1 and IsIcon <> 1 and Popup <> 1 and [Language] = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' and ScreenName = ' + Ti.App.SQL.safeSQL(screenName) + ' and AccessLevel = ' + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + ' order by ScreenName, DisplayNo');
			...
			if(commonObj.dbDataRows.isValidRow()) {
				...
				if(commonObj.menuIconList != null && commonObj.menuIconList != undefined && commonObj.menuIconList != ''){
					if(commonObj.menuIconList.length > 0){
						var _LoopLen1 = commonObj.menuIconList.length;
...
//COMMON.Log('loadFormConfig START -> ' + new Date().getTime());		
		...
		for (var abc = 0; abc < length; abc++) {
			...
			if (formdata.FieldControl == 'WIDGET' || formdata.FieldControl == 'EXPANDABLELISTVIEW') {
			...
			if (formdata.FieldControl == 'WIDGET') {
				...
				try{
					...
					Ti.API.info('ExpListViewScreenName -> '+ screenName+"_FORM_EXPLISTVIEW_HEADER_"+formdata.fieldName);
					...
					Ti.API.info('ExpListViewHeaderQry '+ExpListViewHeaderQry);
					...
					ExpTableView.addEventListener("click", function(e) {
						...
						Ti.API.info('e.index --> ' + e.index + 'e.row.opened : ' + e.row.opened + ' - e.row.isparent : ' + e.row.isparent + ' - e.row.subView : ' + e.row.subView);
						...
						if(rowIsParent){
							if(rowOpened) {
								//REMOVE
								//layout[_iCnt].SubList = 0;
								//Ti.API.info('e.row.SubList.length --> ' + e.row.SubList.length);
								...
								if(rowSubView == true){
									...
									e.row.opened = true;
									ExpTableView.data[0].rows[e.index + 1].height = ExpTableView.data[0].rows[e.index + 1].ActHeight;//150 * Ti.App.dHeightRatio;
								}else{
									//Add teh children.
									Ti.API.info('ExpTableView --> ' + ExpTableView);
									Ti.API.info('ExpTableView.data[0] --> ' + ExpTableView.data[0]);
									Ti.API.info('ExpTableView.data[0].rows --> ' + ExpTableView.data[0].rows);
									...
									Ti.API.info('ExpListViewSubQry '+ExpListViewSubQry);
									...
									if(layout.length > 0){
										...
										row1.height = (layout.length-1) * 50 * Ti.App.dHeightRatio;//'auto';//Ti.UI.SIZE;
										row1.ActHeight = (layout.length-1) * 50 * Ti.App.dHeightRatio;//'auto';//Ti.UI.SIZE;
										var subTtableView = Ti.UI.createTableView({
											//style:Titanium.UI.iPhone.TableViewStyle.GROUPED,
											top: 0,
											width : '100%',
											height: (layout.length-1) * 50 * Ti.App.dHeightRatio,//formdata.ValueHeight,//Ti.Platform.displayCaps.platformHeight,
											data: layout,
											screenName : e.row.screenName,
											fieldName : e.row.fieldName,
											//backgroundColor : '#FF0',
											zIndex : 10,
											ActHeight : (layout.length-1) * 50 * Ti.App.dHeightRatio
										});
										...
										subTtableView.addEventListener("click", function(e) {
											...
											Ti.API.info('subTtableView : e.index --> ' + e.index + 'e.row.opened : ' + e.row.opened + ' - e.row.isparent : ' + e.row.isparent + ' - e.row.subView : ' + e.row.subView);
												...
												if(rowOpened) {
													...
													if(rowSubView == true){
														...
														for(var i=0; i<sRow.children[0].children.length; i++){
															...
															/*try{
															Ti.API.info('i--> ' + sRow.children[0].children[i]);
															Ti.API.info('i--> fieldName : ' + sRow.children[0].children[i].fieldName);
															Ti.API.info('i--> fieldControl : ' + sRow.children[0].children[i].fieldControl);
															Ti.API.info('i--> DataMember : ' + sRow.children[0].children[i].DataMember);
															Ti.API.info('i--> text : ' + sRow.children[0].children[i].text);
															Ti.API.info('i--> value : ' + sRow.children[0].children[i].value);
															Ti.API.info('i--> code : ' + sRow.children[0].children[i].code);
															}catch(e){}*/
														}
														Ti.API.info('- - - - - - - - - - - - - - - - - - - - - - - - -');
														Ti.API.info('Ti.App.ExpandedSelectedRowData.length : ' + Ti.App.ExpandedSelectedRowData.length);
														Ti.API.info('Ti.App.ExpandedSelectedRowData : ' + JSON.stringify(Ti.App.ExpandedSelectedRowData.length));
														Ti.API.info('- - - - - - - - - - - - - - - - - - - - - - - - -');
														...
														if(layout.length > 0){
															...
															var subChildtableView = Ti.UI.createTableView({
																...
																//backgroundColor : '#FF0',
																zIndex : 10,
																ActHeight : (layout.length-1) * 50 * Ti.App.dHeightRatio//'auto';//Ti.UI.SIZE;
															});
				...
				if (formdata.FieldControl == 'LINE') {
					...
					view.top = totalHeaderHeight - height;
					...
					if(screenName == 'MainDashboard'){
						Ti.API.info('screenName '+ screenName);
				...
				//  tableView.backgroundcolor = '#ffffff';
					...
					commonObj.query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_LISTVIEW_"+formdata.fieldName);
					if(commonObj.query != null && commonObj.query != undefined && commonObj.query != '') {
						ArrayOperations.prototype.resetRowiIndex();	
						commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_LISTVIEW_'+formdata.fieldName+'_GroupText');
						commonObj.query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_LISTVIEW_'+formdata.fieldName+'_OrderText');
				...
				//COMMON.Log('setselectedRowIndex -> ' + e.row.index);										
 ...
 */					
					...
					_DBformValues = formValues;
					//MULTILINE
					var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_MULTILINE_"+formdata.DefaultValue);
					query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_MULTILINE_'+formdata.DefaultValue+'_GroupText');
					query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_MULTILINE_'+formdata.DefaultValue+'_OrderText');
 ...
 */		
 			if(formdata.FieldControl == 'MULTIPLEPHOTO') {
				...
				var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_MULTIPLEPHOTO_"+formdata.fieldName);
				if(query != null && query != undefined && query != '') {
					query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + "_FORM_MULTIPLEPHOTO_"+formdata.fieldName+"_GroupText");
					query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + "_FORM_MULTIPLEPHOTO_"+formdata.fieldName+"_OrderText");
				}
				...
				var _LoopLen1 = dirItems.length;
				for ( var i=0; i<_LoopLen1; i++ ){					
					if(dirItems[i].indexOf(sItemNo) >= 0){								
						imgArr.push(dirItems[i]);						
					}					
				}
				_LoopLen1 = dirImages.length;
				for ( var i=0; i<_LoopLen1; i++ ){					
				...
				if(imgArr.length > 1){
					...
					var dTabCnt = 0;
					var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_TABGROUP_TAB");
					if(query != null && query != undefined && query != '') {
						query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_TABGROUP_TAB_GroupText');
						query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_TABGROUP_TAB_OrderText');
					}
					//var db = commonObj.dbConnectionObj.createDataBaseConnection();
					dbDataRows = Ti.App.dbConn.execute(query);
					while (dbDataRows.isValidRow()) {
					...
					while (dbDataRows.isValidRow()) {
						...
						field.addEventListener('click', function(e) {
							try {
								...
								Ti.API.info('e.source.screenName : '+ e.source.screenName +', e.source.code : ' + e.source.code + ', : e.source.index : ' + e.source.index);
				...
				try{
					...
					var dTabCnt = 0;
					var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_TAB_"+formdata.fieldName);
					if(query != null && query != undefined && query != '') {
						query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_TAB_'+formdata.fieldName+'_GroupText');
						query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_TAB_'+formdata.fieldName+'_OrderText');
					}
				...
				view.add(sDashView);
			}else if (formdata.FieldControl == 'LABEL') {
			test = (test == null || test == undefined) ? '' : test;
			Ti.API.info('4738 label');
			...
			*/
				...
				try{
					...
					if(ArrayOperations.prototype.getAllRows(0).length > 0){
						...
						if(commonObj.tblLen >0){
							...
							for(var _i=0 ; _i<fieldNameslength; _i++){
								...
								pColumnwidth = (commonObj._field.columnWidth != undefined && commonObj._field.columnWidth != null && commonObj._field.columnWidth !='') ? commonObj._field.columnWidth : 0;
								if (pColumnwidth > 0 && commonObj._fieldControl == 'EDITABLETEXTBOX') {
									if(mView != null && mView != undefined){
    		...
    		//if(formdata.screenName == 'Sales Items' &&  && Ti.App.EnableFromSearchAddItem == true){
			if((formdata.screenName == 'Sales Items' || formdata.screenName == 'Sales Return Items' || formdata.screenName == 'Sales Exchange Items' || formdata.screenName == 'Stock Order')  && Ti.App.EnableFromSearchAddItem == true){
    			ScanBtn.backgroundImage = Ti.App.ImageCacheObj.getImage('MobileSalesCache','PlusButton.png');
    		}else{
    			ScanBtn.backgroundImage = Ti.App.ImageCacheObj.getImage('MobileSalesCache','BarcodeReader.png');
    		}
			ScanBtn.top = valueTop;
			ScanBtn.borderWidth = 1;
			...
			ScanBtn.field = field;
			ScanBtn.screenName = formdata.screenName;
    		...
    		ScanBtn.addEventListener('click', function(e) {
				...
				if((e.source.screenName == 'Sales Items' || e.source.screenName == 'Sales Return Items' || e.source.screenName == 'Sales Exchange Items' || e.source.screenName == 'Stock Order' ) && Ti.App.EnableFromSearchAddItem == true){
					try{
						//mController.openAddItemScreen();
						mController.menuItemClicked(-1,'','ADDITEM','ADDITEM');
					}catch(e){}
				...
				if(Ti.App.ScanBarCode == true){
					...
					BarcodeData.addEventListener('success', function (e) {
						Ti.App.SearchBarcodeActivated = false;    
						...
						if(sQRCode != ''){
							...
							Ti.API.info('Ti.App.UISearchField --> ' + Ti.App.UISearchField);
				...
				for(var i=0;i<barcodeFormatarr.length;i++){
					try{				
						...
						Ti.API.info("dictionary  "+barcode_format[i]);
					}catch(e){
					    Ti.API.info("error "+e);
				    }
				}
    		...
    		var ClearBtn= commonObj.BasicButtonObj.createButton("X", "9%", formdata.ValueHeight, formdata.VFontSize, '#e8e8e8');
    		//ClearBtn.backgroundImage = Ti.App.ImageCacheObj.getImage('MobileSalesCache','ClearSearchButton.png');
			ClearBtn.top = valueTop;
			ClearBtn.borderWidth = 1;
			...
			field.addEventListener('focus', function(e) {
				try {
					Ti.API.info('Focus e.source.bReadOnly '+e.source.bReadOnly +' fieldname :'+ e.source.fieldName );
					Ti.App.bformTextFieldLostFocus = false;
					if(e.source.bReadOnly == true){
						Ti.API.info('5230 blur');
						this.blur();
						return "";
					}
					this.currentFocus = true;
			...
			field.addEventListener('blur', function(e) {
				try {
					Ti.API.info('Blur e.source.bReadOnly '+e.source.bReadOnly +' fieldname :'+ e.source.fieldName );
					if(Ti.App.bformTextFieldLostFocus == true){						
						Ti.App.bformTextFieldLostFocus = false;
						return "";
					}
			...
			field.addEventListener('return', function(e) {
				try {
					Ti.API.info('return - fieldname :'+ e.source.fieldName );
					//e.source.blur();
					//return "";
					this.currentFocus = false;
					Ti.App.bFocusedTxtfield = null;
			...
			Ti.API.info('commonObj.mScreenNameComboQry '+commonObj.mScreenNameComboQry);
			commonObj.mScreenNameComboQryTxt = this.getQueryConfigByScreenNameWithOrderText(commonObj.mScreenNameComboQry);
			Ti.API.info('commonObj.mScreenNameComboQryTxt '+commonObj.mScreenNameComboQryTxt);
			//qry = mView.formatQueryString(qry, mScreenName);
			if (commonObj.mScreenNameComboQryTxt != undefined) {
			...
			commonObj.mScreenName = formdata.screenName + '_FORM_COMBOBOX_' + formdata.fieldName;
			commonObj.qry = this.getQueryConfigByScreenName(commonObj.mScreenName);
			if(commonObj.qry != null && commonObj.qry != undefined && commonObj.qry != '') {
				commonObj.qry += ' ' + Titanium.App.Properties.getString('QueryConfig_'+commonObj.mScreenName+'_GroupText');
				commonObj.qry += ' ' + Titanium.App.Properties.getString('QueryConfig_'+commonObj.mScreenName+'_OrderText');
			}
			...
			if(formdata.DefaultValue == 'EMPTY'){
				dateLabel.value = '';
				dateLabel.code = '';
			}else if(test == null || test == "" || test == undefined){
				dateLabel.value = this.FormatDataMember(Ti.App.DATEFORMAT.dbDateFormatSQLite(new Date()), sDataMemberType);//Ti.App.DATEFORMAT.dbDateFormat(new Date());
				if(Ti.App.MDY == true){//if(Ti.App.UOMType == 2){
				...
				dateLabel.value = this.FormatDataMember(Ti.App.DATEFORMAT.dbDateFormatSQLite(new Date()), sDataMemberType);
				if(Ti.App.MDY == true){//if(Ti.App.UOMType == 2){
			...
			if(commonObj.bEventFlag == true){				
				...
				dateLabel.addEventListener('change', function(e) {
					try {
						Ti.API.info('value :'+e.source.value+ ' Code '+e.text);
			...
			view.add(timeLabel);
			var img = new BasicImageView().createImageView(null, '/images/downArrow.png', (Math.ceil(formdata.ValueHeight/1.5)), (Math.ceil(formdata.ValueHeight/2)));
			img.top = (formdata.ValueHeight - (Math.ceil(formdata.ValueHeight/2))) / 2;
			if(bIsAndroid){
                img.left = (_lblTotalWidth - ((Math.ceil(formdata.ValueHeight/1.5))/100)*19*(parseFloat(_lblTotalWidth)/parseFloat(view.dWidth))) + '%';
			}else{
			...
			function FrmCheckBoxEvent(e) {
				...
				try{
					Ti.API.info('this.iIndex --> ' + this.iIndex + ' : this.defaultText --> ' + this.defaultText);
					
					var sDefaultText = COMMON.CheckString(this.defaultText);
					Ti.API.info('sDefaultText --> ' + sDefaultText);
					if(sDefaultText != ''){
						...
						if(arrOptions.length > 0){
							...
							for(var dGroupOptionCnt=0; dGroupOptionCnt<arrOptions.length; dGroupOptionCnt++){
								Ti.API.info('this.fieldName : ' + this.fieldName + ' != arrOptions[dGroupOptionCnt] : ' + arrOptions[dGroupOptionCnt]);
								Ti.API.info('GET ROW COMPONWNT : ' + arrOptions[dGroupOptionCnt] + ' : ' + ArrayOperations.prototype.getFormComponent(arrOptions[dGroupOptionCnt]));
								try{
			...
			view.add(toggleBtn);
		}else if(formdata.FieldControl == 'IMAGE') {
		...
		if(align == 2){  Ti.UI.TEXT_ALIGNMENT_LEFT;
			...
			img.addEventListener('click', function(e) {
				try {
					...
					if(this.bImgFound == false){
						...
						Ti.API.info('6648 qry ---> ' + qry);
						var dbDataRows = Ti.App.dbConn.execute(qry);
						if(dbDataRows.isValidRow()) {
							dbDataRows.close();
							//db.close();	
			...
			var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_BUTTONGROUP_"+formdata.fieldName);
			if(query != null && query != undefined && query != '') {
				query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_BUTTONGROUP_'+formdata.fieldName+'_GroupText');
				query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_BUTTONGROUP_'+formdata.fieldName+'_OrderText');
			}
			//var db = commonObj.dbConnectionObj.createDataBaseConnection();
			dbDataRows = Ti.App.dbConn.execute(query);
			var dTabCount = 0;
			...
			if(formdata.DefaultValue == ''){
				...
				if(formdata.DefaultValue != '/Map/Route Map'  && formdata.DefaultValue != '/Map/Offlinemap' && formdata.DefaultValue != '' && formdata.DefaultValue != '/Map/Cust Map' && formdata.fieldName !="ShowcaseFile"){
					try{
						...
						if(screenName.toUpperCase() == 'NewCustomer'.toUpperCase())
						{
							_URL = '';
					var file = (bIsAndroid) ? Ti.Filesystem.getFile(Ti.Filesystem.externalStorageDirectory, formdata.DefaultValue) : Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, formdata.DefaultValue);
					if (file.exists()) {
	...
	loadFormValueData : function(qry, formDataArray) {
		...
		try {
			...
			while (commonObj.dbDataRows.isValidRow()) {
				...
				for (var iIndex = 0; iIndex < commonObj.len; iIndex++) {
					commonObj.fieldName = commonObj.dbDataRows.fieldName(iIndex);
					commonObj.obj = {};
...
alert(replaceALL(str1, str2, true)); 
	...
	getSearchConfig : function(screenName) {
        ...
        dbDataRows = Ti.App.dbConn.execute('SELECT * FROM SearchConfig where ScreenName = ' + Ti.App.SQL.safeSQL(screenName) + ' and  Language = ' + Ti.App.SQL.safeSQL(Ti.App.sLanguage) + ' order by ScreenName, DisplayNo ');
        while (dbDataRows.isValidRow()) {
            screenName = dbDataRows.fieldByName('ScreenName');
            searchdata = {};
            searchdata.ScreenName = screenName;//dbDataRows.fieldByName('ScreenName');
		...
		for (var ctr = 0; ctr < searchDetailslength; ctr++) {
			if (searchDetails[ctr].IsSearch == 1 || searchDetails[ctr].IsSearch == 'true') {
				...
				if (button.searchControl == 'COMBOBOX' || button.searchControl == 'COMBOBOXFILTER') {
					/*var mScreenName = searchDetails[ctr].ScreenName + '_SEARCH_COMBOBOX_' + searchDetails[ctr].FieldName;
					var qry = this.getQueryConfigByScreenName(mScreenName);
					if(qry != null && qry != undefined && qry != '') {
				...
				button.addEventListener('click', function(e) {
					...
					if (this.searchControl == 'TEXTBOX') {
						...
						if(this.searchControl == 'COMBOBOXFILTER'){
							Ti.API.info(this.screenName + '_SEARCH_COMBOBOX_' + this.fieldName);
							...
							Ti.API.info('Search Config ComboBox Query ---> ' + commonObj.qry);
					    ...
					    try{
					    	...
					    	Ti.API.info("sScreenName "+sScreenName);
					    	
    					    var query =  ArrayOperations.prototype.getQueryConfigByScreenName(sScreenName+"_SEARCH_BUTTON_"+this.fieldName);
                            if(query != null && query != undefined && query != '') {
                                ArrayOperations.prototype.resetRowiIndex(); 
                                query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + sScreenName + '_SEARCH_BUTTON_'+this.fieldName+'_GroupText');
                                query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + sScreenName + '_SEARCH_BUTTON_'+this.fieldName+'_OrderText');
                                //COMMON.Log('chart query ---> ' + query);
	...
	createComboBoxData : function(qry) {
		...
		try{
			if (qry != undefined) {
				...
				//Ti.API.info('createComboBoxData -> ' + JSON.stringify(commonObj.dbDataRows1));
	...
	createUITest : function(screenName, iIndex, item) { //7051 - 5187 : TOTAL LINE - 1864
		...
		for (var ctr = 0; ctr < HeaderDetailslength; ctr++) {
			...
			try {
				...
				if(screenName == "Task List"){
					Ti.API.info('7400 Task List');
					//Ti.API.info('screenName : ' + screenName);
					label.touchEnabled = true;
					label.addEventListener('click', function(e) {
						Ti.App.columnClicked = e.source.DataMember;
						//Ti.API.info('AP Ti.App.columnClicked '+Ti.App.columnClicked);
					});
				}
...
//COMMON.Log('LoadData CreateUI stmpActiveScreenName : ' + stmpActiveScreenName + ' - screenName : ' + screenName);
		...
		row.backgroundColor = 'transparent';//sCreateUIRowBgColor;
				...
				try{
					if (dataMemberType != 'STRING') {
						...
						if (dataMemberType == 'DATE') {
							if(item.fieldByName(HeaderDetailsObj.DataMember) == undefined || item.fieldByName(HeaderDetailsObj.DataMember) == ''){
								Ti.API.info('DATE empty');
								dataValue = '';
							}else{
								dataValue = Ti.App.DATEFORMAT.formatDate(item.fieldByName(HeaderDetailsObj.DataMember), commonObj.DateFormatString);								
							}
							Ti.API.info('DATE dataValue :'+dataValue);
							//dataValue = Ti.App.DATEFORMAT.formatDate(item.fieldByName(HeaderDetailsObj.DataMember), commonObj.DateFormatString);
							...
							if(HeaderDetailsObj.columnWidth != 0 && HeaderDetailsObj.columnWidth != '0' && HeaderDetailsObj.columnWidth != ''){
								...
								sDataMember = sDataMember.split("$$");
								if(sDataMember.length>0){
									Ti.API.info("sDataMember[1] "+sDataMember[1] +' '+sDataMember[0]);
...
//COMMON.Log('CreateUI LINENO - 7629 : ' + new Date().getTime());				
				if (mFieldControl.name == 'LABEL' || mFieldControl.name == 'Label') {//Label
					...
					if(HeaderDetailsObj.DataMember.toUpperCase()=='SEPARATOR'){
						dataValue = '/';
					}
					...
					*/
					//if(screenName == "Task List"){
						...
						label.addEventListener('touchstart', function(e) {
							Ti.App.Pricecolumfocused = false;
							Ti.API.info('AP Ti.App.columnClicked ' + e.source.DataMember);
							Ti.App.columnClicked = e.source.DataMember;
							try{
								if(mView != null && mView != undefined){
							...
							if(sCondArr.length > 0 ){
								...
								try{
									...
									for(var condCtr=0; condCtr<sCondArrlength; condCtr++){
										...
										Ti.API.info('dColorConfigRowIndex ---> ' + this.fieldName + ' - ' + sCondArr[condCtr].FieldName);
										if(this.fieldName == sCondArr[condCtr].FieldName){
											if(sCondArr[condCtr].Condition == '>'){
												if(DataMemberValue > ConditionFieldValue){
													...
													Ti.API.info('> CForeColor  '+CForeColor+' CBackColor'+CBackColor);
												...
												Ti.API.info('DataMemberValue '+DataMemberValue +' ConditionFieldValue '+ConditionFieldValue);
												if(DataMemberValue == ConditionFieldValue){
													CForeColor = ArrayOperations.prototype.argbToRGB(sCondArr[condCtr].CForeColor);
													CBackColor = ArrayOperations.prototype.argbToRGB(sCondArr[condCtr].CBackColor);
													Ti.API.info('= CForeColor  '+CForeColor+' CBackColor'+CBackColor);
										...
										Ti.API.info(' ConditionFieldValue '+ConditionFieldValue);
										...
										Ti.API.info('this.backgroundColor  '+this.backgroundColor+' this.color'+this.color);
	...
	field.borderWidth = 1;
					...
					field.addEventListener('change', function(e) {
						...
						if(e.source.DataMember.toUpperCase() == 'BULKQTY' || e.source.DataMember.toUpperCase() == 'PACKQTY'){
							//Ti.API.info('7606 listvaluechanged'+((e.source.value % 1)===0));
						...
						try {
							...
							Ti.API.info('e.source.DataMember :'+e.source.DataMember +' this.dPreValue '+ this.dPreValue +' e.value '+ e.value +' - '+ e.source.value +' - '+ this.value);
					...
					field.addEventListener('blur', function(e) {
						...
						this.editable = true;
						this.focusable = true;
						Ti.API.info('this.valueChanged : '+this.valueChanged);
						//if(this.valueChanged == true || Ti.version >= '7.5.0'){//7.5.0
						if(this.valueChanged == true){//} || Ti.version >= '7.5.0'){	//7.5.0
							Ti.App.bReadOnlyRowTextField = true;
							this.valueChanged = false;
							try{
								...
								if(sCondArr.length > 0 ){
									...
									for(var condCtr=0; condCtr<sCondArrlength; condCtr++){
										...
										Ti.API.info('ConditionFieldValue '+ConditionFieldValue +':CForeColor '+CForeColor+' CBackColor'+CBackColor);
										if(ConditionFieldValue == ''){
												Ti.API.info(' 7820 DataMemberValue '+DataMemberValue + ' === ConditionFieldValue'+ConditionFieldValue);
...
if(bRowComponentBorder == true){
	...
	field.borderWidth = 1;
					...
					//COMMON.Log('List Config ComboBox Query ---> ' + qry);
					try{
...
if(bRowComponentBorder == true){
	...
	dateLabel.borderWidth = 1;
					...
					dateLabel.addEventListener('click', function(e) {
						...
						//COMMON.Log('iIndex :' + this.iIndex + ' - screenName:' + this.screenName + ' - fieldName:' + this.fieldName + ' - searchType:' + this.searchType);
						Ti.App.columnClicked = e.source.DataMember;
						var datepicker = new DatePicker();
						datepicker.setController(mController);
						try {
							//COMMON.Log(e.source.iIndex + ' - ' + this.iIndex);
							Ti.API.info(' this.DataMemberType '+this.DataMemberType + ' dateLabel : '+dateLabel.text);
							if(this.DataMemberType='DATE' && dateLabel.text == '' ){
								dateLabel.text = Ti.App.DATEFORMAT.formatDate(new String(new Date()), Ti.App.ARRAYOPERATION.getSystemValue('DateFormatString'));
								Ti.API.info('dateLabel :'+dateLabel.text);
							}
          ...
          checkBox.defaultText = HeaderDetailsObj.defaultText;
					...
					if (Ti.Platform.name == 'android') {
						Ti.API.info('CLICK : CheckBoxEvent');
						checkBox.addEventListener('click', checkboxevent1);
					} else {
						Ti.API.info('CHANGE : CheckBoxEvent');
						...
						try{
							...
							if(sDefaultText != ''){
								...
								if(arrOptions.length > 0){
									...
									for(var dGroupOptionCnt=0; dGroupOptionCnt<arrOptions.length; dGroupOptionCnt++){
										Ti.API.info('this.fieldName : ' + this.fieldName + ' != arrOptions[dGroupOptionCnt] : ' + arrOptions[dGroupOptionCnt]);
										Ti.API.info('GET ROW COMPONWNT : ' + arrOptions[dGroupOptionCnt] + ' : ' + ArrayOperations.prototype.getRowComponent(0, this.iIndex, arrOptions[dGroupOptionCnt]));
										try{
											if(this.fieldName != arrOptions[dGroupOptionCnt]){
					...
					OptionGroupView.value = (dataValue == null || dataValue == undefined || dataValue == "" || dataValue == 0 || dataValue == "0" || dataValue == "false" || dataValue == false) ? false : true;
					...
					var checkBox1 = new BasicCheckBox().createBasicCheckBox(dataValue, HeaderDetailsObj.allignment);
					checkBox1.value = (dataValue == null || dataValue == undefined || dataValue == "" || dataValue == 0 || dataValue == "0" || dataValue == "false" || dataValue == false) ? false : true;
					...
					var checkBox2 = new BasicCheckBox().createBasicCheckBox(dataValue, HeaderDetailsObj.allignment);
...
if(dataValue == "unreadMsg.simg" || dataValue == "unreadMsg.png"){
						...
						//if(dLineIndex > 0){
							...
							ImgCtrlView.addEventListener('touchstart',function(e){
								...
								Ti.API.info("e.source.dataValue --> " + e.source.dataValue);
								//mView.disabledFormScrollView();
								try{
									imgPathArr = [];
		...
		try {
			if (fieldNames.indexOf(fieldName) > -1) {
				...
				if (rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'OPTION') {
					...
					try{
						pColumnwidth = rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].columnWidth;
						pColumnwidth = (pColumnwidth == undefined || pColumnwidth == null || pColumnwidth == '') ? 0 : pColumnwidth;
						Ti.API.info('pColumnwidth --> ' + pColumnwidth);
						Ti.API.info('fieldControl --> ' + rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl);
						if(Ti.version >= '7.5.0' && (pColumnwidth == 0 || pColumnwidth == '0') && (rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX')){
						//if(Ti.version >= '7.5.0'){	
							rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fireEvent('change');//7.5.0
						}
					}catch(e){
						Ti.API.info('updateColumnData 7.5.0 Error :'+e);
					}
					/*if(Ti.version >= '7.5.0' && rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].ColumnWidth == 0 && (rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX')){
					   rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fireEvent('change');//7.5.0
					}*/
				} else {
			...
			if (fieldNames.indexOf(fieldName) > -1) {
				...
				if (rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'OPTION') {
					...
					try{
						...
						//if(Ti.version >= '7.5.0' && (rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX')){
					   	//if(Ti.version >= '7.5.0'){
				   			rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].fireEvent('change');//7.5.0
				   		}
				   	}catch(e){}
	...
	updateCurrentRow : function(fieldName, mText) {
		...
		if (fieldNames.indexOf(fieldName) > -1) {
			if (Ti.App.currentRow.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || Ti.App.currentRow.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX') {
				...
				//pColumnwidth = (Ti.App.currentRow.children[0].children[fieldNames.indexOf(fieldName)].ColumnWidth != undefined && rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].ColumnWidth != null && rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].ColumnWidth !='') ? rowObj[rowIndex].children[0].children[fieldNames.indexOf(fieldName)].ColumnWidth : 1;
				//Ti.API.info('pColumnwidth --> ' + pColumnwidth);
				//Ti.API.info('fieldControl --> ' + Ti.App.currentRow.children[0].children[fieldNames.indexOf(fieldName)].fieldControl);
	...
	getFormListViewComponentValue : function(sFieldVal) {
		...
		if (index > -1) {
			...
			if(frmListView.selectedTblRowIndex > -1){
				...
				if (fieldControl == 'TEXTBOX' || fieldControl == 'EDITABLETEXTBOX' || fieldControl == 'OPTION') {
		...
		try{
			...
			if (commonObj.index > -1) {
				...
				if (fieldComponent != null && fieldComponent != undefined) {
					...
					Ti.API.info(' fieldControl:' + fieldControl);
					...
					if (fieldControl == 'PASSWORD' || fieldControl == 'SEARCH' || fieldControl == 'TEXTBOX' || fieldControl == 'TEXTAREA' || fieldControl == 'EDITABLETEXTBOX' || fieldControl == 'OPTION' || fieldControl == 'LABEL') {	
						if (fieldControl == 'PASSWORD'){
						Ti.API.info(' fieldComponent.value :' + fieldComponent.value);
						}
			...
			for(var i = 0; i<arrFieldslength; i++){
				...
				if (commonObj.index > -1) {
					...
					if (fieldComponent != null && fieldComponent != undefined) {
						...
						if (fieldControl == 'TEXTBOX' || fieldControl == 'TEXTAREA' || fieldControl == 'EDITABLETEXTBOX' || fieldControl == 'OPTION' || fieldControl == 'LABEL') {	
							obj = {};
	...
	setFormComponentValue : function(fieldName, sValue) {
		...
		if (commonObj.index > -1) {
			...
			if (fieldComponent != null && fieldComponent != undefined) {
				...
				if (fieldControl != 'LISTVIEW' && fieldControl != 'MULTILINE' && fieldControl != 'SWITCH' && fieldControl != 'IMAGE' && fieldControl != 'DASHBOARD' && fieldControl != 'OPTION'){
					fieldComponent.text = sValue;
					fieldComponent.value = sValue;
					fieldComponent.hintText =sValue;
				}
...
//LOG.debug('Arrayoperation - getColumnData : fieldName - ' + fieldName , ' AvailableMemory : ' + COMMON.availableMemoryInMB());
		...
		if (fieldNames.indexOf(fieldName) > -1 && rowIndex > -1) {
			...
			}else if ( commonObj.fieldControl == 'COMBOBOX') {	
	...
	getCurrentRowData : function(fieldName) {
		...
		if (fieldNames.indexOf(fieldName) > -1) {
			...
			if (commonObj.rows.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || commonObj.rows.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX' || commonObj.rows.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'OPTION') {
	...
	getCurrentRowDataWithDataMemberType : function(fieldName) {
		...
		if (fieldNames.indexOf(fieldName) > -1) {
			...
			if (rowObj.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'TEXTBOX' || rowObj.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'EDITABLETEXTBOX' || rowObj.children[0].children[fieldNames.indexOf(fieldName)].fieldControl == 'OPTION') {
	...
	getTotalPage : function(qry){
		...
		//commonObj.db = commonObj.dbConnectionObj.createDataBaseConnection();
		Ti.API.info('getTotalPage qry '+qry);
		/*commonObj.dbDataRows = Ti.App.dbConn.execute(qry);
		while(commonObj.dbDataRows.isValidRow()){
			...
			try{
				...
				if(_strValue == 0){
					...
					if(dataMemberType == 'AMOUNTNUMBERFORMAT'){
						...
						//Ti.API.info('10442 sValue '+sValue);
						sValue = this.getSystemValue('Currency') + ' ' + sValue;
					}else{
						sValue = sValue;	
					}
					...
					if(_strValue.indexOf('/') > -1 || _strValue.indexOf(' / ') > -1){
						...
						if(arr.length > 0){ 
							...
							for(var i = 0; i<arr.length; i++){
								...
								if(dataMemberType == 'AMOUNTNUMBERFORMAT'){
									...
									//Ti.API.info('10464 sValue '+_tmpDataVal);
									if(i==0){
						...
						if(arr.length > 0){ 
							...
							for(var i = 0; i<arrlength; i++){
								...
								if(dataMemberType == 'AMOUNTNUMBERFORMAT'){
									_tmpDataVal = _tmpDataVal.toFixed(this.getSystemValue('AmountRoundingDigits'));
									//Ti.API.info('10492 sValue '+_tmpDataVal);
									if(i==0){
										_dataVal = this.getSystemValue('Currency') + " " +_tmpDataVal;
									}else{
										_dataVal = _dataVal + " / " + this.getSystemValue('Currency') + " " + _tmpDataVal;
						...
						if(dataMemberType == 'AMOUNTNUMBERFORMAT'){
							sValue = sValue.toFixed(this.getSystemValue('AmountRoundingDigits'));
							//Ti.API.info('10516 sValue '+sValue);
							sValue = this.getSystemValue('Currency') + ' ' + sValue;
						}else{
							sValue = sValue;
						}
...
//COMMON.Log('LoadData CreateUI Start Time21 : ' + new Date().getTime());						
	...
	getViewByScreenName : function(_sScreenName){
		try{
			var view = Ti.App.BasicViewObj.createBasicView(null, 'transparent', '100%', '100%', null, null, null, null, 'vertical');
			...
			tableView.backgroundColor = 'transparent';
			view.add(tableView);
			var mainQuery = ArrayOperations.prototype.getQueryConfigByScreenName(_sScreenName);
			if (mainQuery != 'null' && mainQuery != null && mainQuery != undefined && mainQuery != 'undefined') {
				try {
					mainQuery += ' ' + Titanium.App.Properties.getString('QueryConfig_' + _sScreenName + '_GroupText');
					mainQuery += ' ' + Titanium.App.Properties.getString('QueryConfig_' + _sScreenName + '_OrderText');
	...
	getDashboardView : function(dHeight, dWidth, sFieldName, sDataMember, sScreenName){
		...
		Ti.API.info('DASHBOARD : ' + sScreenName+'_FORM_DASHBOARD_'+sFieldName);
		...
		Ti.API.info('getDashboardView qry'+qry);
		totalmenu = 0;	
		dbDataRows = Ti.App.dbConn.execute(qry);
		while (dbDataRows.isValidRow()) {
		...
		dbDataRows = Ti.App.dbConn.execute("select * from workflowConfig WHERE Access = " + access + " and level = 1 Order By OrderNo");
		while (dbDataRows.isValidRow()) {
			tmpCnt++;
			objWF = {};
		...
		try{
			...
			//dbDataRows = Ti.App.dbConn.execute("Select * from GridFunctions where lower(PageName) = lower(" + Ti.App.SQL.safeSQL(pageName) + ") and Access = " + access + " and Language = " + Ti.App.SQL.safeSQL(this.getSystemValue('Language')) + " order by PageNo, RowNo, ColNo");
			dbDataRows = Ti.App.dbConn.execute(GridFunQry);
			while (dbDataRows.isValidRow()) {
		...
		try{
			...
			//Ti.API.info('dGridCols1 '+dGridCols +' '+dGridRows);
			//db = commonObj.dbConnectionObj.createDataBaseConnection();
			...
			dbDataRows.close();
			//Ti.API.info('dGridCols2 '+dGridCols +' '+dGridRows);
			...
			dbDataRows.close();
			//Ti.API.info('dGridCols3 '+dGridCols +' '+dGridRows);
			//dGridCols = Math.ceil(dGridCols/2);
			//dGridRows = Math.ceil(dGridRows/3);
			//Ti.API.info('dGridCols4 '+dGridCols +' '+dGridRows);
			...
			//Ti.API.info('dGridCols5 '+dGridCols +' '+dGridRows);
			//db.close();
			//DEMO
			//ParkFood
			//RE-VISIT-13Jul2018
			//dGridCols = 3;
			//dGridRows = 3;
			//Ti.API.info("Ti.App.CustNo grid "+Ti.App.CustNo);
			...
			//Ti.API.info('dGridCols '+dGridCols +' '+dGridRows);
			//dGridCols = 3;
			//dGridRows = 3;
				...
				for (var j = 0; j < y; j++) {
					for (var k = 0; k < x; k++) {
						...
						thisView1.addEventListener('click', function(e) {
							...
							Ti.API.info("sDashScreenName "+sDashScreenName);
							/*if(Ti.App.BlockTransaction == true){
							    commonObj._dbDataRows = Ti.App.dbConn.execute("select * from Functions where viewOnly = 0 and  code ='"+sDashScreenName+"'");
							...
							Ti.API.info('item clicked sstartendwork '+sstartendwork);
							...
							if (sDashScreenName.toUpperCase() != 'Synchronization'.toUpperCase() && sDashScreenName.toUpperCase() != "SendData".toUpperCase()) {
                                ...
                                try{
                                    Ti.API.info('startwork sstartendwork '+sstartendwork);
                                    sstartendwork = (sstartendwork != undefined && sstartendwork != null && sstartendwork != '') ? sstartendwork : '';
                                    Ti.API.info('startwork sstartendwork '+sstartendwork);
                                    if (sstartendwork != ''){
                                    	if(Ti.App.DBCOMMON.CheckData("select * from custvisit where Transtype='STARTWORK' and Date(TransDate) = Date('now','localtime','start of day') and Agentid="+ Ti.App.SQL.safeSQL(Ti.App.agentID) + "") == true){
							                Ti.App.DBCOMMON.ExecuteSQL("Update systemlist set systemvalue=1 where code ='StartEndWork' ");
							            }
                                    ...
                                    Ti.API.info('startwork end '+sstartendwork);
                                }catch(e){
                                    Ti.API.info('error :'+e);
                                }
    						}
							...
							Ti.API.info("commonObj.bWorkFlowEnabled "+commonObj.bWorkFlowEnabled);
							if(commonObj.bWorkFlowEnabled == true){//START commonObj.bWorkFlowEnabled = true;
								...
								commonObj._dbDataRows = Ti.App.dbConn.execute(commonObj._qry);
								if(commonObj._dbDataRows.isValidRow()){
								...
								try{
									...
									commonObj._dbDataRows1 = Ti.App.dbConn.execute("select * From WorkFlowConfig WHERE lower(FunctionName) = lower(" + Ti.App.SQL.safeSQL(sWFScreenName) + ") and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + " and CompletedON < Date('now','localtime','start of day')"); 
									if(commonObj._dbDataRows1.isValidRow()){
										var _dt = new Date();
										_dt = Ti.App.DATEFORMAT.dbDateFormatSQLite(_dt);
										Ti.App.dbConn.execute("Update WorkFlowConfig SET Status = 0, CompletedOn = " + Ti.App.SQL.safeSQL(_dt) + " WHERE lower(FlowType) = 'main' and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
										Ti.App.dbConn.execute("Update WorkFlowConfig SET Status = 0, CompletedOn = " + Ti.App.SQL.safeSQL(_dt) + " WHERE lower(FlowType) = lower('Customer') and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
								...
								if(commonObj.dbDataRows.isValidRow()){
									...
									commonObj._dbDataRows = Ti.App.dbConn.execute("Select * from WorkFlowConfig  WHERE lower(FlowId) = lower(" + Ti.App.SQL.safeSQL(sWFScreenName) + ") and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
									if(commonObj._dbDataRows.isValidRow()){
										var params = {};
							...
							//var sDashScreenName = this.ScreenName;
							Ti.API.info("LINE 13047 "+sDashScreenName);
							Titanium.App.Properties.setString('SYNC_SCREEN', '');
								if(sDashScreenName.toUpperCase() == 'StartActivity'.toUpperCase()){
									...
									return "";
								}else if(sDashScreenName.toUpperCase() == 'CaptureGPS'.toUpperCase()){
									Ti.Geolocation.purpose = 'Get Current Location';
									//Ti.Geolocation.accuracy //Ti.Geolocation.accuracy_BEST;
									Ti.Geolocation.distanceFilter = 1 ;
									Titanium.Geolocation.getCurrentPosition(function(e) {
								...
								//}else if(sDashScreenName.toUpperCase() == 'Customers'.toUpperCase() && access == 3){
									...
									this.opacity = 1;						
									return true;
								}else if(sDashScreenName.toUpperCase() == 'Customers'.toUpperCase() || sDashScreenName.toUpperCase() == 'OpenPrintSummary'.toUpperCase() || sDashScreenName.toUpperCase() == 'PrintDocuments'.toUpperCase() || sDashScreenName.toUpperCase() == 'NewCustomer'.toUpperCase() || sDashScreenName.toUpperCase() == 'RouteMap'.toUpperCase()){	
									try{
										try{
										Ti.API.info("sDashScreenName1 "+sDashScreenName +' '+COMMON.CheckBooleanField(ArrayOperations.prototype.getSystemValue('CustAgentMDTNo')));
										}catch(e){Ti.API.info("Error "+e);}
										var cnt = 0;
										var tmp_custid = "";
										if(COMMON.CheckBooleanField(ArrayOperations.prototype.getSystemValue('CustAgentMDTNo'))){
											Ti.API.info("sDashScreenName2 "+sDashScreenName);
											commonObj.dbDataRows = Ti.App.dbConn.execute("select COUNT(CustAgentID) as cnt,CustAgentID From CustAgent WHERE AgentId = " + Ti.App.SQL.safeSQL(ArrayOperations.prototype.getSystemValue("PDAID")) + " ");
											Ti.API.info("select COUNT(CustAgentID) as cnt,CustAgentID From CustAgent WHERE AgentId = " + Ti.App.SQL.safeSQL(ArrayOperations.prototype.getSystemValue("PDAID")) + " ");
											while (commonObj.dbDataRows.isValidRow()) {
												cnt = commonObj.dbDataRows.fieldByName('cnt');
												tmp_custid = commonObj.dbDataRows.fieldByName('CustAgentID');
												commonObj.dbDataRows.next();
											}
											commonObj.dbDataRows.close();
											if(cnt == 1){
												Ti.App.agentID = tmp_custid;
												Ti.API.info("tmp_custid "+tmp_custid);
											}
											...
											if(e.index == 0){
												...
												*/
												...
												var buffer1 = Ti.createBuffer({
												    value: "น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล",
												    type: Ti.Codec.CHARSET_UTF8//CHARSET_ISO_LATIN_1 //CHARSET_UTF16
												});
												Ti.API.info( 'buffer1 -> ' + buffer1);
												
												var buffer2 = Ti.createBuffer({
												    value: "น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล",
												    type: Ti.Codec.CHARSET_UTF16
												});
												Ti.API.info( 'buffer2 -> ' + buffer2);
												
												var buffer3 = Ti.createBuffer({
												    value: "น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล",
												    type: Ti.Codec.CHARSET_UTF16LE
												});
												Ti.API.info( 'buffer3 -> ' + buffer3);
												
												var buffer4 = Ti.createBuffer({
												    value: "น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล",
												    type: Ti.Codec.CHARSET_UTF16BE//CHARSET_ISO_LATIN_1 //CHARSET_UTF16
												});
												Ti.API.info( 'buffer4 -> ' + buffer4);
												
												var buffer5 = Ti.createBuffer({
												    value: "น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล",
												    type: Ti.Codec.CHARSET_ASCII//CHARSET_ISO_LATIN_1 //CHARSET_UTF16
												});
												Ti.API.info( 'buffer5 -> ' + buffer5);
												
												file.write('! 0 200 200 ' + dPrnterH + ' 1\r\nON-FEED IGNORE\r\n'
														 + 'T TAHOMA8P.CPF 0 30 0 \r\nLEFT\r\nCENTER\r\n'
														 + 'T TAHOMA8P.CPF 0 30 38 $$$$$$$$$$$$$\r\nLEFT\r\nCENTER\r\n'
														 + 'T TAHOMA8P.CPF 0 30 76 TESTPAGE\r\nLEFT\r\nCENTER\r\n'
														 + 'T TAHOMA8P.CPF 0 30 114 $$$$$$$$$$$$$\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 150 '+buffer1+'\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 190 '+buffer2+'\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 240 '+buffer3+'\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 290 '+buffer4+'\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 320 '+buffer5+'\r\nLEFT\r\n'
														 + 'T TAHOMA8P.CPF 0 30 360 น้ำตาลไซรับ ลิน กลิ่นวนิลา 300 มล\r\nLEFT\r\n'
														 + 'PRINT\r\n');
								...
								if (sDashScreenName.toUpperCase() == 'Stock Order'.toUpperCase()) {	
                                    ...
                                    if(bStockRequestByLocation == true && (Ti.App.accessLevel == 2 || Ti.App.accessLevel == '2')){
                                        Ti.App.dbConn.execute("UPDATE FormConfig SET HeaderHeight = 58, ValueHeight = 58 WHERE Screenname = 'Stock Order' and FieldName = 'Location'");    
                                    }else{
                                        Ti.App.dbConn.execute("UPDATE FormConfig SET HeaderHeight = 0, ValueHeight = 0 WHERE Screenname = 'Stock Order' and FieldName = 'Location'");
                                    }
							...
							var sOrderNo = "";
							if (this.functionName == 'Sales' || this.functionName == 'OpenOrderTaking' || this.functionName == 'Exchange') {
								/*F&N-POINT : 58
			 					* 
								Credit Limit Check :
                                    ...
                                    if (COMMON.isPlatformAndroid()) {
                                        ...
                                        alertDialog.addEventListener('click', function(e) {
                                            if (e.index == 0) {// clicked "OK"
                                                try{
                                                    Ti.API.info('Ti.App.dtDeliveryDate --> ' + Ti.App.dtDeliveryDate);
							...
							if (Titanium.Network.networkType == Titanium.Network.NETWORK_NONE) {
								...
								if(downloadTransConfig_List.length > 0) {
									var downloadTransConfig_Listlength = downloadTransConfig_List.length;
									for(var ctr=0; ctr<downloadTransConfig_Listlength; ctr++){
...
//COMMON.Log('DASHBOARD OPEN SCREEN : ' + new Date().getTime());											
							try{
								...
								var params = {};
								params.sFunctionName = this.functionName;
								Ti.API.info('11557 ob'+sDashScreenName);	
	...
	updateWorkFlowStatus : function(sCurrentScreenName, sAccessLevel, sWorkFlowID){
		...
		Ti.App.dbConn.execute("Update WorkFlowConfig Set Status = 1, CompletedOn = " + Ti.App.SQL.safeSQL(commonObj._dt) + " WHERE FunctionName = " + Ti.App.SQL.safeSQL(sCurrentScreenName) + " and Access = " + Ti.App.SQL.safeSQL(sAccessLevel) + " and FlowId = " + Ti.App.SQL.safeSQL(sWorkFlowID));
	...
	checkWorkFlow : function(){
		...
		setTimeout(function(){
			...
			commonObj._qry = "Select * from Functions WHERE lower(ScreenName) = lower("+Ti.App.SQL.safeSQL(Ti.App.currentScreenName)+") limit 0,1";
			commonObj._dbDataRows = commonObj.db.execute(commonObj._qry);
			if(commonObj._dbDataRows.isValidRow()){
 ...
 */			
 			if(Ti.App.sWorkFlowID != ''){
				commonObj._dt = new Date();
				commonObj._dt = Ti.App.DATEFORMAT.dbDateFormatSQLite(commonObj._dt);
				commonObj.db.execute("Update WorkFlowConfig Set Status = 1, CompletedOn = " + Ti.App.SQL.safeSQL(commonObj._dt) + " WHERE FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + " and FlowId = " + Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID));
...
//COMMON.Log('Ti.App.sWorkFlowID : ' + Ti.App.sWorkFlowID + ' - Ti.App.WorkFlowLevel ' + Ti.App.WorkFlowLevel);
			...
			commonObj.dbDataRows = commonObj.db.execute(commonObj.qry);
			if(commonObj.dbDataRows.isValidRow()){
...
//COMMON.Log('winObj.bStartFlowVisited ---> ' + winObj.bStartFlowVisited);
				if(winObj.bStartFlowVisited == true){
					...
					if(COMMONMODEL.CheckData("SELECT * FROM WorkFlowConfig WHERE (status = 0 or status is null) and FlowId = " +Ti.App.SQL.safeSQL(commonObj.dbDataRows.fieldByName('FlowId'))+" and  Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel))){
...
//"select * from WorkFlowConfig Where lower(FlowId) = lower('FlowVisitShowCase') and  Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel)  + " and FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Level = " + Ti.App.SQL.safeSQL(Ti.App.WorkFlowLevel)  + " Order By OrderNo limit 0,1";					
					...
					*/
					...
					if(dCnt == 1){
						...
						commonObj._dt = new Date();
						commonObj._dt = Ti.App.DATEFORMAT.dbDateFormatSQLite(commonObj._dt);
						commonObj.db.execute("Update WorkFlowConfig Set Status = 1, CompletedOn = " + Ti.App.SQL.safeSQL(commonObj._dt) + " WHERE FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + " and NextFunctionName = ''");
						commonObj.db.execute("DELETE FROM WorkFlowStatus WHERE FlowId = "+Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID)+" and  FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
						commonObj.db.execute("INSERT INTO WorkFlowStatus (FlowId, FunctionName, Access, Status, CompletedOn, Uploaded) VALUES ("+Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID)+", " + Ti.App.SQL.safeSQL(sFunctionName) + ", " + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + ",1," + Ti.App.SQL.safeSQL(commonObj._dt) + ",0)");
			...
			if(NextFunctionName != ""){
				...
				try{
					...
					if(Ti.App.sWorkFlowID != ""){
						...
						commonObj.db.execute("DELETE FROM WorkFlowStatus WHERE FlowId = "+Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID)+" and  FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
						commonObj.db.execute("INSERT INTO WorkFlowStatus (FlowId, FunctionName, Access, Status, CompletedOn, Uploaded) VALUES ("+Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID)+", " + Ti.App.SQL.safeSQL(sFunctionName) + ", " + Ti.App.SQL.safeSQL(Ti.App.accessLevel) + ",1," + Ti.App.SQL.safeSQL(commonObj._dt) + ",0)");
						commonObj.db.execute("Update WorkFlowConfig Set Status = 1, CompletedOn = " + Ti.App.SQL.safeSQL(commonObj._dt) + " WHERE FunctionName = " + Ti.App.SQL.safeSQL(Ti.App.sWorkFlowID) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
						//db.close();
					}else{
						commonObj._dt = new Date();
						commonObj._dt = Ti.App.DATEFORMAT.dbDateFormatSQLite(commonObj._dt);
						commonObj.db.execute("Update WorkFlowConfig Set Status = 1, CompletedOn = " + Ti.App.SQL.safeSQL(commonObj._dt) + " WHERE FunctionName = " + Ti.App.SQL.safeSQL(sFunctionName) + " and Access = " + Ti.App.SQL.safeSQL(Ti.App.accessLevel));
					}
					//db.close();
				}catch(e){}
...
//db.close();
	...
	checkColorConfigWithValidation : function(){
 	...
 	UpdateFrmTblData : function(tableView, screenName, fieldName){
 		var query =  ArrayOperations.prototype.getQueryConfigByScreenName(screenName+"_FORM_LISTVIEW_"+fieldName);
		if(query != null && query != undefined && query != '') {
			ArrayOperations.prototype.resetRowiIndex();	
			query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_LISTVIEW_'+fieldName+'_GroupText');
			query += ' ' + Titanium.App.Properties.getString('QueryConfig_' + screenName + '_FORM_LISTVIEW_'+fieldName+'_OrderText');
			//COMMON.Log('chart query ---> ' + query);
